...
class Akspurchase extends CI_Controller {
    ...
    { 
      ...
      $data['product_id_get'] = $this->db->query("SELECT * FROM AKS_PURCHASE_CART WHERE pur_id='".$bid."' AND status=0 ")->result_array();
    ...
    public function view_table_pro()
    {
        $plid = $this->input->post('id'); // Use input library instead of $_POST directly
        ...
        if($getid) {
            ...
            $data['cart_view'] = $this->Akspurchase_model->get_cart_view_production($product_id);
            ...
            echo ""; // Handle case where no data is found
        }
    }
    ...
    public function aks_new_purchase(){
        ...
        $data['category_production']  = $this->db->query("SELECT * FROM AKSPRODUCT_CATEGORY WHERE Status='Y' ")->result_array();
    ...
    public function add_cart_list()
    {
        ...
        $data['category_list1']  = $this->db->query("SELECT * FROM AKSPRODUCT_CATEGORY  ORDER BY AKSCATEGORY_ID ASC")->result_array();
        ...
        <td> 
            <div> 
                <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');" name="prd_wgt[]" id="prd_wgt'.$plid.'" onkeyup="price_update('.$plid.')" value="0" class="form-control form-control-lg form-control-solid fs-7 product_validation" style="width:90px;"> 
            </div>
        </td>
        <td> 
            <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');"  class="form-control form-control-lg form-control-solid fs-7 d-inline-block" style="width:70px;" name="lb_prd_price[]" id="lb_prd_price'.$plid.'" value="'.$prd_pur_price.'" onkeyup="price_update('.$plid.')" > / '.$unit_price.'
                 <input type="hidden" name="prd_unit[]" id="prd_unit'.$plid.'" value="'.$unit_price.'">
        </td>
        <td id="lbl_price_tot'.$plid.'" align="end">0.00</td>
        <td>
            <a href="javascript:;" name="'.$plid.'"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm btnDelete" data-bs-toggle="modal" >
    ...
    public function category_select(){
        ...
        if($clid != 'all'){
          $result  = $this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE AKS_CAT_NAME = '".$clid."'AND STATUS='Y' AND PRODUCT_TYPE ='P' AND IS_PURCHASE ='1' ")->result_array();
        }else{
        ...
        // echo $clid;                    
         ...
         if($plist['AKS_PRD_IMG']!=''){
          ...
          $htmlcontent = $htmlcontent.'<div class="col-lg-3 fv-row fv-plugins-icon-container" onclick="add_cart('.$plist["AKS_PRD_MID"].')"><a href="javascript:;" id="add_cart" class="btn btn-active-primary px-2 py-2"><div class="image-input-wrapper overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover w-90px h-80px" style="background-image: url('.$image_url.'); border-radius: 10px; "> </div> </a><div class="d-flex flex-column"> <a href="javascript:;" id="add_cart" class="text-gray-600 text-hover-primary fs-8">'.$plist["AKS_PRD_NAME"].'</a></div><span  class="fs-6 fw-bold"><i class="fa-sharp fa-solid fa-indian-rupee-sign eyc fs-6" id="add_cart"></i>'.$plist["AKS_PUR_PRICE"].'/'.$plist["AKS_PRD_WT"].' g</span></div>';
    ...
    public function category_select_pro(){
        ...
        // echo $clid;                    
         ...
         if($plist['AKS_PRD_IMG']!=''){
          ...
          $htmlcontent = $htmlcontent.'<div class="col-lg-3 fv-row fv-plugins-icon-container" onclick="add_cart_pro('.$plist["AKS_PRD_MID"].')"><a href="javascript:;" id="add_cart_pro" class="btn btn-active-primary px-2 py-2"><div class="image-input-wrapper overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover w-90px h-80px" style="background-image: url('.$image_url.'); border-radius: 10px; "> </div> </a><div class="d-flex flex-column"> <a href="javascript:;" id="add_cart_pro" class="text-gray-600 text-hover-primary fs-8">'.$plist["AKS_PRD_NAME"].'</a></div><span  class="fs-6 fw-bold"><i class="fa-sharp fa-solid fa-indian-rupee-sign eyc fs-6" id="add_cart_pro"></i>'.$plist["AKS_PUR_PRICE"].'/'.$plist["AKS_PRD_WT"].' g</span></div>';        
    ...
    public function add_cart_list_pro()
    {
        $plid = $_POST['id'];
        $count = $_POST['count'];
        $data['cart_list'] = $this->Akspurchase_model->get_cart_list($plid);
        ...
        // $oniput='oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1'");" ';
         $row = '
                    <tr  name="count">
                        <td>'.($count+1).'<input type="hidden" name="item_count_hidden_pro[]'.$count.'" id="item_count_hidden_pro" value="'.($count+1).'">
                        </td>
                        <td class="table_tool_purchase">
                            <div class="d-flex mb-1">
                                <div class="d-flex align-items-center">
                            ...
                            <input type="hidden" name="prd_id_hidden_pro[]" id="prd_id_hidden_pro'.$plid.'" value="'.$plid.'">
                        </td>
                        
                        <td> 
                            <div> 
                                <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');" name="prd_wgt_pro[]" id="prd_wgt_pro'.$plid.'" onkeyup="price_update_pro('.$plid.')" value="0" class="form-control form-control-lg form-control-solid fs-7 product_validation" style="width:90px;"> 
                            </div>
                        </td>
                        <td> 
                            <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');" maxlength="9"  class="form-control form-control-lg form-control-solid fs-7 d-inline-block" style="width:70px;" name="lb_prd_price_pro[]" id="lb_prd_price_pro'.$plid.'" value="'.$prd_pur_price.'" onkeyup="price_update_pro('.$plid.')" > / '.$unit_price.'<input type="hidden" name="prd_unit_pro[]" id="prd_unit_pro'.$plid.'" value="'.$unit_price.'">
                        </td>
                        <td id="lbl_price_tot_pro'.$plid.'" align="end">0.00</td>
                        <td>
                            <a href="javascript:;" name="'.$plid.'"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm btnDeletepro" data-bs-toggle="modal" >
                                <span class="svg-icon svg-icon-3">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5 9C5 8.44772 5.44772 8 6 8H18C18.5523 8 19 8.44772 19 9V18C19 19.6569 17.6569 21 16 21H8C6.34315 21 5 19.6569 5 18V9Z" fill="currentColor"></path>
                                        <path opacity="0.5" d="M5 5C5 4.44772 5.44772 4 6 4H18C18.5523 4 19 4.44772 19 5V5C19 5.55228 18.5523 6 18 6H6C5.44772 6 5 5.55228 5 5V5Z" fill="currentColor"></path>
    ...
    public function purchase_save()
    {
        ...
        $data['production_chk']    = $this->input->post('production_chk') ?$this->input->post('production_chk') :0;
        $data['mackingcharges']    = $this->input->post('mackingcharges');
        $data['total_amt_pro']     = $this->input->post('total_amt_pro');
        $data['net_amt_pro']       = $this->input->post('net_amt_pro');
        $data['tallyamount']       = $this->input->post('tallyamount_hidden');

        if ($data['production_chk']==1) {

            $stk_status = 'Purchase Production';
        }else{
            $stk_status = 'Purchase';
        }
        ...
        $data['prd_wgt_pro']        = $this->input->post('prd_wgt_pro');
        $data['pro_pur_price']      = $this->input->post('lb_prd_price_pro');
        $data['pro_unit']           = $this->input->post('prd_unit_pro');
        $data['pro_prd_id']         = $this->input->post('prd_id_hidden_pro'); 

         // print_r($data['prd_wgt_pro']);

        $count=count($data['prd_wgt_pro']);
        $count1 = $count;

        $data['pro_prd_count']   = $count1;

        for($i=0;$i<$count;$i++){

        $wgt        =  $data['prd_wgt_pro'][$i] ;

        $unit_price =  $data['pro_unit'][$i];

        $purprice   = $data['pro_pur_price'][$i];
        ...
        $total_price = $tot;
        // com
        $cart_insert=$this->db->query("INSERT INTO AKS_PURCHASE_PRODUCTION_CART
            (product_id,product_wgt,pur_price,price,pur_id,status) values ('".$data['pro_prd_id'][$i]."','".$data['prd_wgt_pro'][$i]."','".$data['pro_pur_price'][$i]."','".$total_price."','".$data['pur_id']."',0)");

        // print_r($total_price);
        // print_r($cart_insert);

          $prd_ids = $data['pro_prd_id'][$i];
         
          $data['crt_qty'] =($this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE AKS_PRD_MID = '".$prd_ids."' ")->row());
          $crt_qty     =  $data['crt_qty'] ? $data['crt_qty']->PRD_CUR_QTY: 0;
          $crt_qty_tot =  $crt_qty;
          $wgts        =  $data['prd_wgt_pro'][$i] ;
          $bal_qty     =  $crt_qty_tot - $wgts ;
          $stk_bal_qty =  $bal_qty;
          $min_stock   =  $data['crt_qty']?$data['crt_qty']->AKS_MIN_STK:0;
            ...
            // com
            $updatestock = $this->db->query("
                                                INSERT INTO AKS_STOCK
                                                (
                                                prd_id
                                               ,stk_date
                                               ,stk_cur_qty
        ...
        if ($purchase_result) {
            ...
            if  ($old_data !='') {
                    ...
                    // // print_r($old_credit.'====$old_credit<br>');
                    // // print_r($old_debit.'========$old_debit<br>');
                    // // print_r($old_open.'=========$old_open<br>');
                    // // print_r($old_bal.'======$old_bal<br>');
                    // print_r($data['tallyamount'].'======$data["tallyamount"]<br>');
                    // print_r($data['net_amt_pro'].'======$data["net_amt_pro"]<br>');


                    if ($data['production_chk']==1) {

                    //         if ($old_credit<$data['tallyamount']) {

                    //            $debited   = $old_debit - ($data['tallyamount'] - $old_credit);
                    //            $credited  = 0;
                    //            $balance   = $old_bal - $debited ;

                    //             print_r($old_credit.'====oldcr <'.$data['tallyamount'].'===tallyamount Condition 1<br>');
                    //             print_r($credited.'======$credited1<br>');
                    //             print_r($debited.'======$debited1<br>');
                    //             print_r($balance.'======$balance1<br>');
                              
                    //         }

                    //         if ($old_credit>$data['tallyamount']) {

                    //             $credited   = $old_credit - $data['balance_cash'];
                    //             $debited    = 0;
                    //             $balance    = $old_bal + $credited ;

                    //             print_r($old_credit.'====oldcr >'.$data['tallyamount'].'===tallyamount Condition 2<br>');

                    //                print_r($credited.'======$credited2<br>');
                    //                print_r($debited.'======$debited2<br>');
                    //                print_r($balance.'======$balance2<br>');
                    //         }

                    //         if($data['tallyamount']==0){

                    //             $credited   = $old_credit;
                    //             $debited    = $old_debit - $data['net_amt_pro'];
                    //             $balance    = $old_bal;

                    //             print_r($old_credit.'====oldcr == '.$data['tallyamount'].'===tallyamount Condition 3<br>');

                    //                print_r($credited.'======$credited3<br>');
                    //                print_r($debited.'======$debited3<br>');
                    //                print_r($balance.'======$balance3<br>');
                    //         }
                    //         $sup_data = [                                
                    //                     'credit'   =>$credited,
                    //                     'debit'    =>$debited,
                    ...
                    //                     ];
                        ...
                        if ($old_credit<$data['pur_net_amt']) {

                           $debited   =  ($data['balance_cash'] - $old_credit) + $old_debit;
                           $credited  = 0;
                           $balance   = $old_bal - $debited ;
    ...
    public function supplier_list_det()
    {
      ...
      public function purchase_edit($id){
        ...
        $data['edit'] = $this->Akspurchase_model->get_purchase_edit_list($id);


        if ($data['edit']) {

          $payment_id = $data['edit']->pur_id;
        ...
        // print_r($data['edit']);
        // print_r($data['cash_detail']);
        // print_r($data['upi_detail']);
        // print_r($data['cheque_detail']);
        // exit();
        
        $this->load->view('aks_purchase/aks_edit_purchase',$data);
    ...
    {
        $id = $_POST['id'];
        $data['cart_list'] = $this->Akspurchase_model->get_edit_cart_list($id);
        ...
        foreach ($data['cart_list'] as $count => $clist) {
            ...
            $prd_list = $this->Akspurchase_model->get_cart_list($prd_id);
            $prd_img = $prd_list->AKS_PRD_IMG;
            $prd_name = $prd_list->AKS_PRD_NAME;
            $prd_hsn = $prd_list->HSN_CODE;
            $prd_pur_price = $prd_list->AKS_PUR_PRICE;
            ...
            if ($prd_img) {
                $image_url = base_url() .'assets/images/aks_product_image/'. $prd_img;
                $img = '<div style="background-image: url('.$image_url.'); border-radius: 10px;"></div>';
            }

            $purchase_price = floatval($clist['product_wgt'] * $clist['pur_price']) / $unit_price;

            $rows .= '<tr name="count">
                        <td>' . ($count + 1) . '<input type="hidden" name="item_count_hidden[]" id="item_count_hidden' . $count . '" value="' . ($count + 1) . '"></td>
                        <td class="table_tool_purchase">
                            <div class="d-flex mb-1">
                                <div class="d-flex align-items-center">
                                    <div class="symbol symbol-35px me-1">
                        ...
                        <td> 
                            <div> 
                                <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');" name="prd_wgt[]" id="prd_wgt' . $prd_id . '"  onkeyup="price_update(' . $prd_id . ')" value="' . $clist['product_wgt'] . '" class="form-control form-control-lg form-control-solid fs-7 product_validation" style="width:90px;"> 
                            </div>
                        </td>
                        <td> 
                            <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');"  class="form-control form-control-lg form-control-solid fs-7 d-inline-block" style="width:70px;" name="lb_prd_price[]" id="lb_prd_price' . $prd_id . '" value="' . $clist['pur_price'] . '" onkeyup="price_update(' . $prd_id . ')" > / ' . $unit_price . '
                            <input type="hidden" name="prd_unit[]" id="prd_unit' . $prd_id . '" value="' . $unit_price . '">
                        </td>
                        <td id="lbl_price_tot' . $prd_id . '" align="end">' . $purchase_price . '</td>
                        <td>
                            <a href="javascript:;" name="' . $prd_id . '"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm btnDelete" data-bs-toggle="modal" >
                                <span class="svg-icon svg-icon-3">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5 9C5 8.44772 5.44772 8 6 8H18C18.5523 8 19 8.44772 19 9V18C19 19.6569 17.6569 21 16 21H8C6.34315 21 5 19.6569 5 18V9Z" fill="currentColor"></path>
                                        <path opacity="0.5" d="M5 5C5 4.44772 5.44772 4 6 4H18C18.5523 4 19 4.44772 19 5V5C19 5.55228 18.5523 6 18 6H6C5.44772 6 5 5.55228 5 5V5Z" fill="currentColor"></path>
            ...
            $arr[] = ['id' => $prd_id, 'amount' => $purchase_price];
        }
    ...
    public function add_edit_pro_cart_list()
    {
        $id = $_POST['id'];
        $data['cart_list'] = $this->Akspurchase_model->get_edit_pro_cart_list($id);
        ...
        foreach ($data['cart_list'] as $count => $clist) {
            ...
            $purchase_price = floatval($clist['product_wgt'] * $prd_pur_price) / $unit_price;

            $row = '
                    <tr name="count">
                        <td>'.($count+1).'<input type="hidden" name="item_count_hidden_pro[]" id="item_count_hidden_pro" value="'.($count+1).'"></td>
                        <td class="table_tool_purchase">
                            <div class="d-flex mb-1">
                                <div class="d-flex align-items-center">
                                    <div class="symbol symbol-35px me-1">
                            ...
                            <input type="hidden" name="prd_id_hidden_pro[]" id="prd_id_hidden_pro'.$prd_id.'" value="'.$prd_id.'">
                        </td>
                        <td> 
                            <div> 
                                <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');" name="prd_wgt_pro[]" id="prd_wgt_pro'.$prd_id.'" onkeyup="price_update_pro('.$prd_id.')" value="' . $prd_pur_price . '" class="form-control form-control-lg form-control-solid fs-7 product_validation" style="width:90px;"> 
                            </div>
                        </td>
                        <td> 
                            <input type="text" maxlength="10" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');" maxlength="9"  class="form-control form-control-lg form-control-solid fs-7 d-inline-block" style="width:70px;" name="lb_prd_price_pro[]" id="lb_prd_price_pro'.$prd_id.'" value="'.$prd_pur_price.'" onkeyup="price_update_pro('.$prd_id.')" > / '.$unit_price.'<input type="hidden" name="prd_unit_pro[]" id="prd_unit_pro'.$prd_id.'" value="'.$unit_price.'">
                        </td>
                        <td id="lbl_price_tot_pro'.$prd_id.'" align="end">' . $purchase_price . '</td>
                        <td>
                            <a href="javascript:;" name="'.$prd_id.'"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm btnDeletepro" data-bs-toggle="modal" >
                                <span class="svg-icon svg-icon-3">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M5 9C5 8.44772 5.44772 8 6 8H18C18.5523 8 19 8.44772 19 9V18C19 19.6569 17.6569 21 16 21H8C6.34315 21 5 19.6569 5 18V9Z" fill="currentColor"></path>
                                        <path opacity="0.5" d="M5 5C5 4.44772 5.44772 4 6 4H18C18.5523 4 19 4.44772 19 5V5C19 5.55228 18.5523 6 18 6H6C5.44772 6 5 5.55228 5 5V5Z" fill="currentColor"></path>
    ...
    public function purchase_update(){
        ...
        $data['pur_date']       = date('Y-m-d',strtotime($this->input->post('pur_date')));
        $data['pur_id']         = $this->input->post('purchase_id_hidden');
        $data['pur_sup']        = $this->input->post('sup_name');
        $data['prd_wgt']        = $this->input->post('prd_wgt');
        $data['pur_price']      = $this->input->post('lb_prd_price');
        $data['unit']           = $this->input->post('prd_unit');
        ...
        if ($data['production_chk']==1){
            ...
            $cart_ids    = $this->input->post('purchase_id_hidden');
            $cart_list   = $this->Akspurchase_model->get_edit_pro_cart_list($cart_ids);

            foreach ($cart_list as $si => $val) {
                $cart_id_get   = $val['id'];
                $product_ids   = isset($val['product_id']) ? $val['product_id'] : ''; // Check if 'product_id' is set
                $updateoldcart = $this->db->query("UPDATE AKS_PURCHASE_PRODUCTION_CART SET status = 1 WHERE id = '".$cart_id_get."' AND product_id = '".$product_ids."'");

                $up_cur_data   = $this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE AKS_PRD_MID = '".$product_ids."' ")->row();

                if ($up_cur_data) { // Check if the product data is retrieved successfully
                    $up_crt_qty     = isset($up_cur_data->PRD_CUR_QTY) ? $up_cur_data->PRD_CUR_QTY : 0;
                    $up_wgts        = isset($val['product_wgt']) ? $val['product_wgt'] : 0;
                    $up_bal_qty     = $up_crt_qty + $up_wgts;
        ...
        // Stock reduce Purchase
        $cart_ids    = $this->input->post('purchase_id_hidden');
        $cart_list_pur   = $this->Akspurchase_model->get_edit_cart_list($cart_ids);

        foreach ($cart_list_pur as $pi => $val) {

          $cart_id_get_pur   = $val['id'];
          $pur_product_ids   = $val['product_id'] ?$val['product_id']:'';
          $updateoldcart = $this->db->query("UPDATE AKS_PURCHASE_CART SET status = 1 WHERE id = '".$cart_id_get_pur."' AND product_id = ".$pur_product_ids);              
                $up_cur_data    = $this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE AKS_PRD_MID = '".$pur_product_ids."' ")->row();
                ...
                $updstkhis      = $this->db->query("INSERT INTO AKS_STOCK
                  ...
                  $up_curt_stk = $stk_up_bal_qty;                 
                  $up_prd_mas  = $this->db->query("UPDATE AKS_PRD_MASTER SET PRD_CUR_QTY='". $up_curt_stk."' WHERE AKS_PRD_MID=". $pur_product_ids);
        ...
        // com
         ...
         // com
        add_notification(date('Y-m-d H:i:s'), '', "Others", "karupatti<br> Update Purchase", $pur_id. ' updated By ' .$_SESSION['username'],'', $_SESSION['USERID'], '0', $pur_id);

        $data['supplier_id'] = $this->input->post('sup_id');

        // com
        $purchase_result = $this->Akspurchase_model->purchase_entry_update($data,$pur_id);
        // $purchase_result = 1;
        ...
        if ($purchase_result) {
            ...
            if  ($old_data !='') {
              ...
              $cash_detail   = $this->db->query("SELECT * FROM payment_inward_outward WHERE bill_no='".$pur_id."' AND type_of_payment='Cash' ")->row();
              $cheque_detail = $this->db->query("SELECT * FROM payment_inward_outward WHERE bill_no='".$pur_id."' AND type_of_payment='Cheque' ")->row();
              $upi_detail    = $this->db->query("SELECT * FROM payment_inward_outward WHERE bill_no='".$pur_id."' AND type_of_payment='UPI' ")->row();

                  if ($cash_detail) {
                    $cash   = $this->Akspurchase_model->upcashsave($data);
                  }else{
                    $cash   = $this->Akspurchase_model->cashsave($data);
                  }

                  if ($cheque_detail) {
                    $cheque = $this->Akspurchase_model->upchequesave($data);
                  }else{
                    $cheque = $this->Akspurchase_model->chequesave($data);
                  }

                  if ($upi_detail) {
                    $upi    = $this->Akspurchase_model->upupisave($data);
                  }else{
                    $upi    = $this->Akspurchase_model->upisave($data);
                  }
            // com
        ...
        if($purchase_result)
        {
            $this->session->set_flashdata('g_success', $pur_id.' New Purchase have been updated successfully...');
        }
        else
        {
           $this->session->set_flashdata('g_err', 'Could not update purchase !');
        }
        redirect('Akspurchase');  
    }
