 ...
 public function get_pur_list($fdate,$tdate,$party_name,$pro_type,$ploat_type,$offset,$page_limit,$vaofill,$amenfill)
    {
        ...
        $result = $this->db->query(" SELECT  * FROM  ( SELECT *, ROW_NUMBER() OVER (ORDER BY id DESC) AS sl FROM realestate_purchase WHERE  property_status='New' AND status='Y' $fdate $tdate $party_name  $pro_type $ploat_type $vaofill $amenfill )N  WHERE  sl BETWEEN $offset AND $page_limit ")->result_array();
   ...
   public function getUsers($search)
  {
   ...
   public function get_fin_years_list()
    ...
    public function get_agent_list($search)
    {
        ...
        $result  = $this->other_db->query("SELECT TOP 30 * FROM ACC_LEDGERS  WHERE  LEDGER_NAME LIKE '".$search.'%'."'  AND GROUP_NAME='RE_AGENT' " )->result_array();
        save_query_in_log();
