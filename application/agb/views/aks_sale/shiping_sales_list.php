				...
				<div class="modal-body pt-0 pb-15 px-5 px-xl-20">
				    ...
				    <div class="row">
				   		...
				   		<label class="col-lg-10 col-form-label fw-bold fs-5" id="delivered_by_scan"></label>
							...
							<div id="sale_view_table_scan">
								
								
							</div>
					
					<div class="row">
					<label class="col-lg-2 col-form-label fw-semibold required fs-6">Delivered&nbsp;By</label>

						<div class="col-lg-2 fv-row fv-plugins-icon-container">
								<select class="form-select form-select-solid" data-control="select2"  data-hide-search="true" data-dropdown-parent="#ship_scanned_model" name="courier_edit" id="courier_edit">
							...
							<div class="col-lg-2 fv-row fv-plugins-icon-container">
								<input type="text" id="charges_enter" name="charges_enter" value="0" maxlength="8" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"   class="form-control form-control-lg form-control-solid" placeholder="Enter Charges" autocomplete="off">

								<input type="hidden" name="form_id" id="form_id">
								<input type="hidden" name="del_supplier_id" id="del_supplier_id">
					...
					<div class="d-flex align-items-center justify-content-end mt-2">
						...
						<button class="btn btn-primary" onclick="courier_delivery();">Delivered</button>
					</div>
		...
		function ok_button_popup()
			{
				...
				success: function(response)
				{
					...
					if (response) {
								...
								$('#total_amount_scan').html(gprice);

								$('#tracking_id_enter').val('');
								$('#charges_enter').val(response.shipment_charges);
								$('#del_supplier_id').val(response.del_supplier_id);
							...
							var supplier = $('#del_supplier_id').val();
							$.ajax({
									type: "POST",
									...
									{	
										  var options = JSON.parse(response);
											...
											options.map((it) => {
											  ...
											  data += '<option value="' + it.LEDGER_NAME + '~' + it.LEDGER_SNO + '" '+select+'>' + it.LEDGER_NAME + '</option>';
											});
		...
		function courier_delivery(){
			...
			if (tracking_id!='' && courier_edit!='') {
					...
					if(courier_edit==''){
						Swal.fire({
											title: 'Error!',
											text: 'Please Select Delivered By..',
											icon: 'error',
											confirmButtonText: 'Okay'
											});
					}
...
<script>
	function view_sale(val)
	{
		...
		success: function(response)
		{
			...
			var div='<div class="row"><label class="col-lg-2 col-form-label fw-semibold fs-6">Tracking ID</label><label class="col-lg-4 col-form-label fw-bold fs-5">'+response.sale_track_id+'</label><label class="col-lg-2 col-form-label fw-semibold fs-6">Charges</label><label class="col-lg-4 col-form-label fw-bold fs-5">'+shipcharge+'</label></div>'
							}
			else{
				var div='';
