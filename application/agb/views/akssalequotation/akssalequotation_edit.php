...
$this->load->model(array("Akssalequotation_model"));
?>
<style>
	.dataTables_scroll
    {
						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Update Sales Quotation
									<!--begin::Separator-->
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
										...
										<div class="card-body py-4">
											<div class="row">
												<div class="col-lg-6">
													<!--  -->
													<form method="POST" autocomplete="off" action="<?php echo base_url(); ?>Akssalequotation/update" id="submit_form" enctype="multipart/form-data" onsubmit="return submit_validation();">
													<!-- <div class="px-4" style="border-radius: 10px;padding-bottom: 10px;box-shadow: 0 5px 10px #00002947;"> -->
														<div class="row">
															<label class="col-lg-2 col-form-label  fw-semibold fs-6">Type</label>
															<div class="col-lg-4 fv-row">
																<select class="form-select form-select-solid" name="aks_sls_qut_type" id="aks_sls_qut_type" data-control="select2" data-hide-search="false" onchange="sales_quote_func();">
																	<option <?php if ($edit->party_type=='party'):echo 'selected'; ?>
																		
																	<?php endif ?>value="party">Party</option>
																	<option <?php if ($edit->party_type=='supplier'):echo 'selected'; ?>
																		
																	<?php endif ?>value="supplier">Supplier</option>
																</select>
																<!-- <label class="fw-bold fs-5 mt-3">< ?php echo $edit->party_type; ?></label> -->
															</div>
															<label class="col-lg-2 col-form-label  fw-semibold fs-6" >Date</label>
															<div class="col-lg-4 d-flex align-items-center" style="padding-left: 0px !important;">
																...
																<?php $date = date('d-m-Y',strtotime($edit->sale_date)) ?>
																<input class="form-control form-control-solid ps-12" name="sale_date" placeholder="Date" id="SALE_entry_add_date" value="<?php echo $date; ?>" />
															</div>
															<!-- <label class="col-lg-2 col-form-label  fw-semibold fs-6">< ?php echo ucfirst($edit->party_type); ?></label> -->
															<!-- <div class="col-lg-4 fv-row fv-plugins-icon-container" id="aks_par_tbox"> -->
															<!-- 	<label class="fw-bold fs-5 mt-3">< ?php echo $edit->sale_party ?></label>
																<input type="hidden" name="party_id_hidden" id="party_id_hidden" value="< ?php echo $edit->id_party??''; ?>"> -->
																<!-- <input type="hidden" name="sale_party"      id="sale_party" value="< ?php echo $edit->sale_party; ?>"> -->
															<!-- </div> -->
															<label class="col-lg-2 col-form-label  fw-semibold fs-6" id="aks_par_label">Party<i class="fa-solid fa-circle-info fs-7 ms-2"  title="AutoComplete Select Party Name"></i></label>
															<div class="col-lg-4 fv-row fv-plugins-icon-container" id="aks_par_tbox">
																<input type="text" name="sale_party" id="sale_party" value="<?php echo $edit->sale_party??'';  ?>" class="form-control form-control-lg form-control-solid me-3" placeholder="Select Party Name" >
																<div class="fv-plugins-message-container invalid-feedback" name="party_err" id="party_err"></div>
																<input type="hidden" name="party_id_hidden" id="party_id_hidden" value="<?php echo $edit->id_party??''; ?>">
															</div>
															<label class="col-lg-2 col-form-label  fw-semibold fs-6" id="aks_sup_label" style="display:none;">Supplier</label>
															...
															<?php
																	if ($edit->party_type != 'supplier') {
																	    $party_id = $edit->id_party ? $edit->id_party : '';
																	    $party_data = $this->db->query("SELECT * FROM PARTIES WHERE IS_KARUPATTI='1' AND PID = '" . $party_id . "'")->row();

																	    if ($party_data) {
																	        // Retrieve address based on record type
																	        if ($party_data->TYPE_OF_RECORD == 'O') {
																	            $CITY = ($party_data->CITY == '') ? '--' : $party_data->CITY;
																	            $ADDRESS2 = ($party_data->ADDRESS2 == '') ? '--' : $party_data->ADDRESS2;
																	            $ADDRESS1 = ($party_data->ADDRESS1 == '') ? '--' : $party_data->ADDRESS1;
																	        } else {
																	            $city_det = $this->db->query("SELECT * FROM CITY WHERE CITYID='" . $party_data->CITY_ID . "'")->row();
																	            $CITY = $city_det ? $city_det->CITYNAME : '--';

																	            $village_det = $this->db->query("SELECT * FROM VILLAGE WHERE VILLAGEID='" . $party_data->VILLAGE_ID . "'")->row();
																	            $ADDRESS2 = $village_det ? $village_det->VILLAGENAME : '--';

																	            $street_det = $this->db->query("SELECT * FROM STREET WHERE STREETID='" . $party_data->STREET_ID . "'")->row();
																	            $ADDRESS1 = $street_det ? $street_det->STREETNAME : '--';
																	        }

																	        // Construct full address
																	        $pin = $party_data->PINCODE ? '-' . $party_data->PINCODE : '';
																	        $address = $ADDRESS1 . ', ' . $ADDRESS2 . ', ' . $CITY . $pin;
																	    } else {
																	        // Handle the case when party data is not found
																	        $address = '---';
																	    }
																	} else {
																	    // Supplier case
																	    $supplier_id = $edit->id_party ? $edit->id_party : '';
																	    $supp_data = $this->Akssalequotation_model->get_sale_sup_view($supplier_id);
																	    $address = '';

																	    if ($supp_data) {
																	        // Construct address from supplier data
																	        $addressParts = [];
																	        if (!empty($supp_data->ADDRESS)) {
																	            $addressParts[] = $supp_data->ADDRESS;
																	        }
																	        if (!empty($supp_data->ADDRESS2)) {
																	            $addressParts[] = $supp_data->ADDRESS2;
																	        }
																	        if (!empty($supp_data->CITY)) {
																	            $addressParts[] = $supp_data->CITY;
																	        }
																	        if (!empty($supp_data->STATE)) {
																	            $addressParts[] = $supp_data->STATE;
																	        }
																	        $address = implode(', ', $addressParts);
																	    } else {
																	        // Handle the case when supplier data is not found
																	        $address = '---';
																	    }
																	}
																	?>

																	<!-- Output the phone, email, and address -->
																	<label class="col-lg-2 col-form-label fw-semibold fs-6">Phone</label>
																	<label class="col-lg-4 col-form-label fw-bold fs-6" id="party_mobile"><?php echo $party_data->PHONE ?? '---'; ?></label>
																	<label class="col-lg-2 col-form-label fw-semibold fs-6">Email</label>
																	<label class="col-lg-4 col-form-label fw-bold fs-6" id="party_email"><?php echo $party_data->EMAIL??'---' ?: '---'; ?></label>
																	<div class="col-lg-2">
																	    <label class="col-form-label fw-semibold fs-6">Address</label>
																	</div>
																	<label class="col-lg-4 col-form-label fw-bold fs-6" id="party_address"><?php echo $address ?></label>	
														...
														<div class="row">
															...
															<div class="col-lg-2 fv-row fv-plugins-icon-container mt-1">
																<input type="text" name="dis_cart_amt" id="dis_cart_amt" class="col-lg-4 form-control form-control-lg form-control-solid fs-4"  onkeyup="cart_prd_totcal(event)" value="<?php echo $edit->sale_dis_amt?$edit->sale_dis_amt:0; ?>" maxlength="5">
															</div>
															<div class="col-lg-4 col-form-label text-center">
														...
														<div class="col-lg-4 d-flex justify-content-end">
															<textarea class="form-control form-control-solid" rows="1" id="remarks" name="remarks" placeholder="Enter Remarks"><?php echo $edit->remarks; ?></textarea>
														</div>
														...
														<input type="hidden" name="sale_id_hidden" id="sale_id_hidden" value="<?php echo $edit->sale_id; ?>">
												...
												<div class="col-lg-6">
													<!-- <div class="px-4" style="border-radius: 10px;padding-bottom: 10px;box-shadow: 0 5px 10px #00002947;"> -->
														...
														<div class="d-flex justify-content-end align-bottom mt-13" style="padding-top:20px !important;">
															...
															<button class="btn btn-primary" type="submit" onclick="submit_validation()" >Update Quotation</button>
														</div>
													</form>
												</div>
												</div>
			...
			<script> 
				...
				function cart_get(){
						...
						if (val !== '') {							
								...
								$.ajax({
									type: "POST",
									url: baseurl+'Akssalequotation/add_edit_cart_list',
									async: false,
									type: "POST",
									data: "id="+val,
									dataType: "html",
