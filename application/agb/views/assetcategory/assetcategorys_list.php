...
<style>
  ...
  .error {
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Asset Category
									<!--begin::Separator-->
								...
								<div class="card-header border-0 pt-6">
									...
									<div class="card-toolbar">
										...
										<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
											<!--begin::Add user-->
											<button type="button" class="btn btn-primary"  data-bs-toggle="modal" data-bs-target="#kt_modal_new_category">
									...
									<table class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2" id="asset_category_table">
										<thead>
											<tr class="text-start text-muted fw-bold fs-7 gs-0">
												<th class="min-w-50px" style="width:10% !important;">S.No</th>
														<th class="min-w-150px" style="width:60% !important;">Category</th>
										...
										<tbody class="text-gray-600 fw-semibold">
											<?php if(isset($assetcategorylist)){ 
												foreach ($assetcategorylist as $i => $list) { ?>
											<tr>
												<td><?php echo $i+1; ?></td>
												...
												<td>
													<label class="form-check form-switch form-check-custom form-check-solid cursor-pointer">
														<input class="form-check-input w-35px h-20px cursor-pointer" type="checkbox" <?php if($list['status']==1){echo "checked";} ?> id="activeunactive_items_<?php echo $i;?>" name="activeunactive_items_<?php echo $i;?>" onchange="activeunactive_items('<?php echo $list['assetcategoryid']; ?>',<?php echo $i;?>)" value="<?php echo $list['status'];?>">
													</label>
												</td>
												<!--begin::Action=-->
												<td>
													<span class="text-end">
														<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit_category" onclick="edit('<?php echo $list['assetcategoryid']; ?>')">
														...
														<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_modal_delete_category" onclick="deleteassetcategory('<?php echo $list['assetcategoryid'];?>')">
		...
		<div class="modal fade" id="kt_modal_edit_category" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
			<!--begin::Modal dialog-->		
			...
			<div class="modal-dialog modal-md">
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body pt-0 pb-15 px-5 px-xl-20">
						...
						<!--end::Heading-->
						<!--   -->
						<form name="item_add_form" enctype="multipart/form-data" method="POST"  action="<?php echo base_url(); ?>Assetcategory/assetcategoryadd" onsubmit="return addvalidation();">
							<div class="row">
								<label class="col-lg-3 col-form-label required fw-semibold fs-6">Category</label>
								<div class="col-lg-9 fv-row fv-plugins-icon-container">
									<input type="text" name="assetcategoryname" class="form-control form-control-lg1 form-control-solid" placeholder="Enter Category Name" id="assetcategoryname" onkeyup="checkunique_add()">
									<div class="fv-plugins-message-container invalid-feedback" id="assetcategoryname_err"></div>
								</div>
							</div>
							<div class="d-flex justify-content-center align-items-center mt-6">
								<button type="reset" class="btn btn-secondary me-3" data-bs-dismiss="modal">Cancel</button>
				...
				<div class="modal-dialog modal-m">
					...
					<div class="modal-content rounded">
						<div class="swal2-icon swal2-warning swal2-icon-show" style="display:flex;">
							...
							<div class="swal2-html-container" id="swal2-html-container" style="display: block;">Are you sure you want to delete this Category <b><?php echo $details->assetcategoryname?$details->assetcategoryname:''; ?></b>  !..?</div>
							<div class="d-flex flex-center flex-row-fluid pt-12">
								<button type="submit" class="btn btn-primary me-3" onclick="removecategory('<?php echo $bid;?>')">Yes</button>
		...
		<script>
			function activeunactive_items(val,ival) {
				...
				$.ajax({
					type:"POST",
					url:baseurl+'Assetcategory/assetcategory_active',
					data:datastring,
					cache: false,
					dataType: "html",
		...
		<script> 
			...
			var uniq = 0;
				...
				{
				   var val          = $('#assetcategoryname').val();
				   var baseurl      = $("#baseurl").val();
				   // alert(val)
				   if (val!='') {
					   $.ajax({
					      type:"POST",
					      url:baseurl+'Assetcategory/checkunique_add',
					      data:"value="+val,
					      cache: false,
					      dataType: "html",
					      success: function(result){
					        if(result==1)
					        {
					            $('#assetcategoryname_err').html('Asset Category already exists!');
					            $('#btnSubmit').prop('disabled', true);
					            uniq = 1;
					        }
					        else
					        {
					            $('#assetcategoryname_err').html('');
			...
			function  addvalidation(){
				...
				var err = 0;
				var assetcategoryname          = $('#assetcategoryname').val();

				// alert(acnts_users);

					if(assetcategoryname.trim() =="")
					{
						
						$('#assetcategoryname_err').html('Enter category Required..!');
						err++;
		...
		<script>
				function edit(val) {
						...
						$.ajax({
						type: "POST",
						url: baseurl+'Assetcategory/edit',
						async: false,
						type: "POST",
						data: "id="+val,
						dataType: "html",
						success: function(response)
						{
							...
							$('#kt_modal_edit_category').empty().append(response);
							$('#kt_modal_edit_category').addClass('show');
						}
						});
				}
			...
			function deleteassetcategory(val){
					...
					$.ajax({
					type: "POST",
					url: baseurl+'Assetcategory/deleteassetcategory',
					async: false,
					type: "POST",
					data: "id="+val,
					dataType: "html",
			...
			{ 
					...
					$.ajax({
					type: "POST",
					url: baseurl+'Assetcategory/delete',
					async: false,
					data:"field="+val,
					success: function(response)
					{
