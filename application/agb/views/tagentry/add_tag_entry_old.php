   ...
   background-color: #ec9629;
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
												...
												<!--begin::Select-->
													...
													<!--<div class="fv-plugins-message-container invalid-feedback" id="company_id_err"></div>-->
														...
														<?php $i=1; foreach($lotentry_details as $ldlist){
															...
															<label class="col-lg-2 col-form-label fw-bold fs-5">:<?php echo $ldlist['item_name']; $item_name=$ldlist['item_name'];$lot_detail_id=$ldlist['id']; ?></label>
															<div class="col-lg-3">
															...
															<label class="col-lg-2 col-form-label fw-semibold fs-6">SubItem Count</label>
													...
													{ $j=$tag_entry->tag_no+1; } 
													 ...
													 for($i=0;$i<$count;$i++){  ?> 
												    	<tr>
																...
																<!--<input type="text" class="form-control_tex form-control-lg_tex form-control-solid_tex" value="<?php echo $i+1; ?>" readonly>-->
												            ...
												            <td>
																...
																$prefix="TAG-";
        $suffix="/23";  $tag_id=$prefix.$tag_no.$suffix;  echo $tag_id; ?>" readonly>
												            </td>
												            <td>
																				<select class="form-select form-select-solid" data-control="select2" data-hide-search="false" name="subitem[]" id="subitem<?php echo $i; ?>">	
																					<option value="">Select SubItem</option>	
																					<?php $subitem_list  = $this->db->query("SELECT * FROM SUBITEM_LIST WHERE jewel_type_id='".$metal_type ."'   ")->result_array(); ?>
																					<?php foreach($subitem_list as $slist){   ?>			
																					<option value="<?php echo $slist['SUBITEM_NAME']  ?>"><?php echo $slist['SUBITEM_NAME']  ?></option>
																					<?php  } ?>
																				</select>
																				<span class="m-form__help text-danger" id="subitem_err<?php echo $i; ?>"></span>
												            </td>
												            <td>
													            <!--<input type="text" class="form-control_tex form-control-lg_tex form-control-solid_tex" value="" name="purity[]" id="purity<?php echo $i; ?>">-->
																<select class="form-select form-select-solid" data-control="select2" data-hide-search="false" name="quality[]" id="quality<?php echo $i; ?>" onchange="item_purity(<?php echo $i;?>);">	
																					...
																					<?php //$subitem_list  = $this->db->query("SELECT * FROM [SUBITEM_LIST] WHERE ITEM_NAME='".$item_name."' ")->result_array(); ?>
																					<?php foreach($purity_list as $plist){   ?>			
																					<option value="<?php echo $plist['ITEMPURITYID'];  ?>"><?php echo $plist['ITEMPURITYNAME'];  ?></option>
																					<?php  } ?>
																				</select>
																				<span class="m-form__help text-danger" id="quality_err<?php echo $i; ?>"></span>
																</td>
																<td>
																<input type="text" class="form-control form-control-lg form-control-solid" value="0" name="purity[]"  id="purity<?php echo $i; ?>" xs>
																<span class="m-form__help text-danger" id="purity_err<?php echo $i; ?>"></span>
																</td>
												            <td>
															...
															<td>
																...
																<span class="m-form__help text-danger" id="hallmark_charges_err<?php echo $i; ?>"></span>
																</td>
															<td>
		...
		function metal_weight(i){
			...
			var metal_weight=parseFloat(net_weight)+parseFloat(wastage_charges/100);
