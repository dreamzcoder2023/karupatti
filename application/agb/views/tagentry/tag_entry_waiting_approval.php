...
background-color: #ec9629 !important;
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									...
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										<span>Company - </span>
										<span>&nbsp;<?php if($company_id==''){ echo "All"; }else{ $company  = $this->db->query("SELECT * FROM COMPANY WHERE ACTIVE='Y' AND COMPANYCODE='". $company_id."' ")->row();
											echo $company->COMPANYNAME; } ?></span>
									</label>
									<span class="h-20px border-gray-400 border-start ms-3 mx-2"></span>
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										<span>Item Type - </span>
										<span>&nbsp;<?php if($jewel_type_fill==''){ echo "All"; }else{ $jewel_type  = $this->db->query("SELECT * FROM jewel_type WHERE  jewel_type_id='". $jewel_type_fill."' ")->row();
										...
										<div class="card-body py-4">
												<div class="mb-5 hover-scroll-x">
													<div class="d-grid">
														<ul class="nav nav-tabs flex-nowrap text-nowrap">
															<li class="nav-item">
																<a class="nav-link  btn btn-active-light btn-color-black-800 btn-active-color-primary rounded-bottom-0 fw-bold fs-6" href="<?php echo base_url(); ?>Tagentry"  >Tagged</a>
															</li>
															<li class="nav-item">
																<a class="nav-link active btn btn-active-light btn-color-black-800 btn-active-color-primary rounded-bottom-0 fw-bold fs-6" data-bs-toggle="tab" href="#kt_tab_pane_2">Waiting  Approval</a>
															</li>
														</ul>
													</div>
												</div>
												<div class="tab-content" id="myTabContent">
													<div class="tab-pane fade show active" id="kt_tab_pane_2" role="tabpanel">
														<div class="row">
															<div class="col-lg-12">
																...
																<div class="col-lg-8">
																	...
																	<div class="row">
																		<label class="col-lg-4 text-success text-center fs-2 fw-bold"><?php echo $tagentry_list1->count; ?></label>
																		<label class="col-lg-4 text-success text-center fs-2 fw-bold"><?php echo $tagentry_list1->t_wgt; ?></label>
																	...
																	<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
																		...
																		<div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
																			<div class="px-7 py-5" data-kt-user-table-filter="form">
																				<form method="POST" id="fill_form" action="<?php echo base_url(); ?>Tagentry/tag_entry_waiting_approval">
																					<div class="scroll-y mh-325px my-1 px-3">
																						...
																						<div class="mb-2 fv-row" id="from_dt" style="display:none;">
																							...
																							<div class="d-flex align-items-center">
																								...
																								<!--end::Svg Icon-->
																								<input class="form-control form-control-solid ps-12" name="from_date_fillter" placeholder="From Date" id="from_date_fillter" value="<?php if($from_date_fillter==''){ echo date("d-m-Y"); } else{ echo $from_date_fillter; } ?>"/>
																						...
																						<div class="mb-2 fv-row" id="to_dt" style="display:none;">
																						    <label class="form-label">To</label>
																								<div class="d-flex align-items-center">
																									...
																									<!--end::Svg Icon-->
																									<input class="form-control form-control-solid ps-12" name="to_date_fillter" placeholder="To Date" id="to_date_fillter" value="<?php if($to_date_fillter==''){ echo date("d-m-Y"); } else{ echo $to_date_fillter; } ?>"/>
														...
														<div class="modal fade" id="kt_modal_export_users_wait" tabindex="-1" aria-hidden="true">
																		<!--begin::Modal dialog-->
																		<div class="modal-dialog modal-dialog-centered mw-650px">
																			...
																			<div class="modal-content">
																				...
																				<div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
																					<!--begin::Form-->
																					<form id="kt_modal_export_users_wait_form" class="form" action="#">
																						<!--begin::Input group-->
														...
														<div class="row">
															<table id="kt_datatable_responsive_not_approved"  class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2 dataTable">
																	<thead>
																	   <tr class="text-start text-muted fw-bold fs-7 gs-0">
																	   	<th class="min-w-80px">Date</th>
																			...
																			<th class="min-w-100px">Wgt(gms) /<br> Stone(gms) /<br> Net Wgt(gms)</th>
																			<th class="min-w-80px">H.M Char /<br>Mak.Char /<br>Wst.Char(%)</th>
																	...
																	<tbody class="text-gray-600 fw-semibold">
																		<?php $i=1; foreach($tagentry_list  as $tlist){ ?>
																			<tr>
																	            <td>
																            		<?php $date_format  = $this->db->query("SELECT * FROM general_settings  ")->row(); $format= $date_format->date_format;echo date("$format", strtotime($tlist['tag_date']));?>
															            		</td>
																	           <td class="ple1">
																	           		<?php 
																	            ...
																	            <td class="ple1">
																	            	<?php 
																				 $item_name  = $this->db->query("SELECT * FROM ITEMS WHERE  SNO='". $tlist['item_name']."' ")->row();
																				if(isset($item_name)) echo $item_name->ITEMNAME;
																				//echo $tlist['item_name']; ?>
																				</td>
																	            <td class="ple1">
																	            	<?php $subitem_name  = $this->db->query("SELECT * FROM SUBITEM_LIST WHERE  SUB_ID='". $tlist['subitem_name']."' ")->row(); if(isset($subitem_name)) echo $subitem_name->SUBITEM_NAME;?>
																				</td>
																	            <td>
																	            	<span><?php echo $tlist['weight']; ?></span>
																	            	<span class="d-block"><?php echo $tlist['stone']; ?></span>
																					<span class="d-block"><?php echo $tlist['net_weight']; ?></span>
																	            </td>
																				<td>
																					<span><?php echo $tlist['hallmark_charges']; ?></span>
																					<span class="d-block"><?php echo $tlist['making_charges']; ?></span>
																					<span class="d-block"><?php echo $tlist['wastage_charges']; ?></span>
																				</td>
																				<td><?php echo $tlist['metal_weight']; ?></td>  
																	          	<td>
																			 		<?php if($tlist['status']=='Y'){ ?>
																					...
																					<?php } ?>
																					<?php if($tlist['status']=='U'){ ?>
																						<label class="col-form-label fw-semibold fs-7">
																							<span style="background-color:#46dcf9;border-radius: 8px;padding: 5px 15px 5px 15px;"    onclick="lot_edit_view('<?php echo $tlist['tag_no']; ?>')">Update Required
																							</span>
																						</label>
																					<?php } ?> 
																					<?php if($tlist['status']=='A'){ ?>
																						<label class="col-form-label fw-semibold fs-7">
																							<span style="background-color:#50cd89;border-radius: 8px;padding: 5px 15px 5px 15px;">Approve
																							</span>
																						</label>
																					<?php } ?>
																				</td>
																				<td>
																					...
																					<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-200px py-3" data-kt-menu="true" style="">
																						<?php if($tlist['tag_no'] =='Y') { ?>	
																						<div class="menu-item px-3">
																							<a href="javascript:;" class="menu-link px-3" data-bs-toggle="modal" data-bs-target="#kt_modal_update_tag_entry" onclick="tag_update_required('<?php echo $tlist['tag_no']; ?>')">Update</a>
																						</div>
																						<?php } ?>
																						<div class="menu-item px-3">
																							<a href="javascript:;" class="menu-link flex-stack px-3" data-bs-toggle="modal" data-bs-target="#kt_modal_verify_tag_entry"   onclick="tag_approve('<?php echo $tlist['tag_no'];?>')">Approve</a>
																						</div>
																					</div>
																				    <?php if(isset($_SESSION['Tag entryEdit'])){ if($_SESSION['Tag entryEdit']==1){?>
																						<span class="text-end">
																							<a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#edit_tag_item"
																							onclick="tag_edit('<?php echo $tlist['tag_no'];?>')" >
															...
															<?php 
																$coun = ceil( $count1/10);
																$c_page = isset($_GET['page1']) ? $_GET['page1'] : 1;
															?>
															<?php $paging_info = get_paging_info1($count1,10,$c_page); ?>
		...
		<div class="modal fade" id="edit_tag_item" tabindex="-1" aria-hidden="true">
	...
	$(document).ready(function() {
        ...
        $(".move_to").on("click", function () {
			value=$(this).val();
			// alert(value);
            $('#filter_form').attr('action', "<?php echo base_url(); ?>Tagentry/tag_entry_waiting_approval?page1="+value);
            $("#filter_form").submit();
            e.preventDefault();
        });
    });
		...
		function submit() {
            let form = document.getElementById("tag_entry_waiting");
            form.submit();
            //alert("Data stored in database!");
        }


		function tag_update_required(val)
			{
				var baseurl= $("#baseurl").val();
				//alert(val);
				$.ajax({
				type: "POST",
				url: baseurl+'Tagentry/tagentry_update_required',
				async: false,
				type: "POST",
				data: "id="+val,
				dataType: "html",
			...
			function closeViewModal()
			{
				...
				$('.modal-backdrop').removeClass('show');
				 window.location.href = baseurl+'Tagentry/tag_entry_waiting_approval';
			}
			</script>
			<script>
			...
			{ 
				...
				$.ajax({
				type: "POST",
				url: baseurl+'Tagentry/update_required?',
				async: false,
			...
			function tag_approve(val)
			{
				var baseurl= $("#baseurl").val();
				//alert(val);
				$.ajax({
				type: "POST",
				url: baseurl+'Tagentry/tagentry_approve',
				async: false,
				type: "POST",
				data: "id="+val,
				dataType: "html",
			...
			function tag_edit(val)
			{
				...
				$.ajax({
				type: "POST",
				url: baseurl+'Tagentry/tagentry_edit',
				async: false,
				type: "POST",
				data: "id="+val,
				dataType: "html",
				success: function(response)
				{
				$('#edit_tag_item').empty().append(response);
				$('#edit_tag_item').addClass('show');
				//$("#kt_modal_delete_role").css("display", "block");
				}
				});
			}
			...
			{
					var baseurl= $("#baseurl").val();
					$('#edit_tag_item').removeClass('show');
					// $('.modal-backdrop').removeClass('show');
					window.location.href = baseurl+'Tagentry/tag_entry_waiting_approval';
			}

			function approvetag(val)
			{ 
				var baseurl= $("#baseurl").val();
				$.ajax({
				type: "POST",
				url: baseurl+'Tagentry/approve',
				async: false,
				data:"field="+val,
				success: function(response)
				{
