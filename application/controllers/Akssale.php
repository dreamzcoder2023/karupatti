...
class Akssale extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['closed_bill_sts'] = $this->input->post('closed_bill_sts') ? $this->input->post('closed_bill_sts'):0;
        // print_r($data['closed_bill_sts']);
        // exit;
        if($data['closed_bill_sts']!='1'){
            $closed_bill_sts =" AND status!='C' ";
        }
        else{
            $closed_bill_sts ='';
        }
        ...
        if($data['dt_fill']=="week"){
           ...
           $data['sale_list'] = $this->Aks_sale_model->get_sale_list_fill($fdate,$tdate,$party_name,$sale_id,$offset,$page_limit,$stsfill,$closed_bill_sts);
           $data['count'] = count($this->db->query("SELECT * FROM AKS_SALE_ENTRY WHERE status!='N'  $fdate $tdate $party_name $closed_bill_sts
           $sale_id $stsfill ORDER BY sid DESC")->result_array());

           

           
            $data['pending'] = $this->db->query("SELECT COUNT(*) as pending FROM AKS_SALE_ENTRY WHERE status ='Y' $fdate $tdate $party_name $closed_bill_sts $sale_id $stsfill ")->row();
            $data['readyforpacking'] = $this->db->query("SELECT COUNT(*) as readyforpacking FROM AKS_SALE_ENTRY WHERE status ='F' $fdate $tdate $party_name $closed_bill_sts $sale_id $stsfill ")->row();
            $data['packed'] = $this->db->query("SELECT COUNT(*) as packed FROM AKS_SALE_ENTRY WHERE status ='S' $fdate $tdate $party_name $closed_bill_sts $sale_id $stsfill ")->row();
            $data['partialpacking'] = $this->db->query("SELECT COUNT(*) as partialpacking FROM AKS_SALE_ENTRY WHERE status ='P' $fdate $tdate $party_name $closed_bill_sts $sale_id $stsfill ")->row();
            $data['hold'] = $this->db->query("SELECT COUNT(*) as hold FROM AKS_SALE_ENTRY WHERE status ='H' $fdate $tdate $party_name $sale_id $closed_bill_sts $stsfill ")->row();


           $data['export_sale'] = $this->db->query("SELECT * FROM AKS_SALE_ENTRY WHERE status != 'N' $fdate $tdate $party_name $closed_bill_sts $sale_id $stsfill ORDER BY sid DESC")->result_array();

        $data['export_sales'] = [];
        foreach ($data['export_sale'] as $key => $value) {
            $export_sale = $value; // Copy the entire $value array
            $party_datas = get_party_details_id($value['id_party'] ?? '');
            $export_sale['party_name']  = $party_datas->NAME ?? '-';
            $export_sale['party_phone'] = $party_datas->PHONE ?? '-';
            $export_sale['party_city']  = $party_datas->CITY ?? '-';
            $export_sale['party_pincode']  = $party_datas->PINCODE ?? '-';
            // Add more fields as needed
            
            $data['export_sales'][] = $export_sale;
        }

        $data['exp_sales'] = json_encode($data['export_sales']); 
        // echo $data['exp_sales'];

           // print_r($data['exp_sales']); 
           // exit;
           $data['sale_id'] = $data['bill_id'];
           $role = $this->db->query("SELECT Role FROM STAFFS WHERE USER_ID = ".$_SESSION['USERID'])->row();

           // Assign the role to the $data array
    ...
    public function sale_list_det()
{
    ...
    foreach ($rows as $row) {
        ...
        $result[] = [
            ...
            "id_party" => htmlspecialchars($id_party, ENT_QUOTES),
            "firstname" => htmlspecialchars($firstname, ENT_QUOTES),
            "address" => htmlspecialchars($address, ENT_QUOTES),
            "shipment_address" => htmlspecialchars($shipment_address, ENT_QUOTES),
    ...
    exit; // or return, depending on how this function is used
}
    ...
    public function shipping_status()
{
    ...
    if ($response) {
        if ($response->status != 'C') { // Check if status is not already 'C' (Cancelled)
            ...
            $this->db->where('sale_id', $sale_id)->where('status !=', 'C')->update('AKS_SALE_ENTRY', $data);

            // Fetch the updated status
            $updated_status = $this->db->select('status')->where('sale_id', $sale_id)->get('AKS_SALE_ENTRY')->row()->status;

            echo '||true||'.$updated_status;
        } else {
            echo '||false||'; // Sale entry is already cancelled
        }
    } else {
        echo '||false||'; // No matching sale entry found
    }
}
   ...
   public function partial_packing()
   {
        ...
        if ($p_wgt > 0) {
            ...
            if ($sale_data->product_wgt == $p_wgt) {
                ...
                $this->db->query("UPDATE  AKS_SALE_ENTRY SET status='C' WHERE sale_id= '".$sale_id."'");
            ...
            //  $sale_data = $this->db->query("SELECT sum(product_wgt) as product_wgt, sum(packed_wgt) as packed_wgt FROM AKS_SALE_CART WHERE sale_id='".$sale_id."' AND status=0 ")->row();
            // if($sale_data->product_wgt == $sale_data->packed_wgt){
   ...
   public function full_packing()
   {
            ...
            $img= '<div style="background-image: url('.base_url().'assets/images/Aks_prd_images/'.$prd_img.'border-radius: 10px;"></div>';
               ...
               $row='<tr  name="count"><td>'.($count+1).'<input type="hidden" name="item_count_hidden[]"  id="item_count_hidden'.$count.'" value="'.($count+1).'"></td>
                    <td class="table_tool">
                     ...
                     <div>
                     <input type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');"  name="sale_wgt[]" id="prd_wgt'.$prd_id.'" value="'.$clist['product_wgt'].'"  onkeyup="price_update('.$prd_id.')" class="form-control form-control-lg form-control-solid fs-7 product_validation">
                     </div></td>
    ...
    public function Weblist(){
        ...
        $data['wp_count'] = count($this->db->query("SELECT * FROM AKS_WEB_SALE_LIST WHERE convert_status = '0'  $wp_fdate $wp_tdate $wp_party_name  ORDER BY date DESC")->result_array());
        
         $data['export_sales'] = json_encode($this->db->query("SELECT * FROM AKS_WEB_SALE_LIST WHERE convert_status = '0'  $wp_fdate $wp_tdate $wp_party_name  ORDER BY date DESC")->result_array());
    ...
    public function wp_change_status(){
        ...
        $id      = $_POST['id'];
        if($status == 'C'){           
            $save = $this->website_sale_save($id);
            // print_r($id);
            if ($save=='2') 
            {
                $status =['status'=>2];
                echo json_encode($status);
                 // $this->session->set_flashdata('g_success',  ' New Sales have been added successfully...');
            }else{
                $status =['status'=>0];
                echo json_encode($status);
                $this->session->set_flashdata('g_err', 'Could not add Website sale. Website product (or) weight is not available. Please check!');   
            }
            // echo $save;
    ...
    {  
        ...
        $data['other_bill']        = $webdata->web_id;
            ...
            $wght_chk = 0;
                foreach ($products_info as $wprd) {
                    ...
                    $wgt_chk       = preg_replace('/[^0-9\-]/', '', $wprd->product_weight);
                    $response_chk  = $this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE WEB_PRD_ID LIKE '%" . $wprd->product_id . "%'")->row();
                        // print_r($wgt); 
        ...
        if($result) {
            ...
            foreach($products_info as $wprd){
                ...
                $wgt_ck                =  preg_replace('/[^0-9\- ]/', '', $wprd->product_weight);
                if (is_numeric($wgt_ck)) {
                    $wgt = $wgt_ck;
                }else{
                    $wgt= 0;
                }
                ...
                // print_r($wgt);
                // exit;
                if (isset($wgt) && isset($unit_price) && isset($purprice) && is_numeric($wgt) && is_numeric($unit_price) && is_numeric($purprice) && $unit_price != 0) {
                    $tot = $wgt / $unit_price * $purprice;
                } else {
                    $tot=0;
                }
                ...
                $reponse = $this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE WEB_PRD_ID LIKE '%" . $wprd->product_id . "%'")->row();

                if ($reponse) {                  
                   
                    $web_product_id = $reponse->AKS_PRD_MID;
                    $res=$this->db->query("INSERT INTO AKS_SALE_CART
                                                    (
                                                    product_id,
                                                    product_wgt,
        ...
        {
            return 2;
            $update = $this->db->query("UPDATE AKS_WEB_SALE_LIST SET status ='C' , convert_status ='1'   WHERE wid ='".$web_id."'");
            ...
            $this->session->set_flashdata('g_success',  $sale_id.' New Sales have been added successfully...');
        }else{
            return 0;
            $this->session->set_flashdata('g_err', 'Could not Add Website sale. Website product (or) weight is not available. Please check!');
            // $this->session->set_flashdata('g_err', 'Could not add sale !');
    ...
    public function website_sale_edit($web_id)
    {   
        ...
        $data['edit'] = $this->db->query("SELECT * FROM AKS_WEB_SALE_LIST  WHERE wid ='".$web_id."'")->row();

        if ($data['edit']) {

            $data['sync_order_id'] = $this->db->query("SELECT * FROM AKS_WEBSITE_SYNC  WHERE ID ='".$data['edit']->sync_id."'")->row();    

            $data['web_order_id'] = $data['sync_order_id']?$data['sync_order_id']->ORDER_ID?$data['sync_order_id']->ORDER_ID:'-':'-';
         ...
         $products_info  = json_decode($data['edit']->products);   
         $prd_chk = 0;
                foreach ($products_info as $wprd) {
                    $webproduct_id = $wprd->product_id;
                    $wgt_chk       = preg_replace('/[^0-9\-]/', '', $wprd->product_weight);
        ...
        // exit;
        if ($prd_chk>0) {
           $this->load->view('aks_sale/aks_web_edit',$data);   
        }
        else{
           $this->session->set_flashdata('g_err', 'Could not Edit Website sale. Website product (or) weight is not available. Please check!');   
           redirect('Akssale/Weblist');
        }       
    }


    public function add_edit_web_cart_list()
    {
        $id      = $_POST['id'];
        $edit    = $this->Aks_sale_model->get_edit_web_cart_list($id);
        // $count = $_POST['count'];
        // print_r($edit);
        // exit();
        $products_info  = json_decode($edit->products);   
            $prd_chk = 0;
             $arr =[];
                $row ="";
                $rows='';
            ...
            if(isset($products_info)){
                foreach ($products_info as $i => $wprd) {
                    $webproduct_id = $wprd->product_id;
                    $web_prd_name  = $wprd->product_name;
                    $wgt_chk       = preg_replace('/[^0-9\-]/', '', $wprd->product_weight);
                    $prd_list  = $this->db->query("SELECT * FROM AKS_PRD_MASTER WHERE WEB_PRD_ID LIKE '%" . $webproduct_id . "%'")->row();
                    $prd_id         = $prd_list->AKS_PRD_MID;
                    $prd_img        = $prd_list->AKS_PRD_IMG;
                    $prd_name       = $prd_list->AKS_PRD_NAME;
                    $prd_hsn        = $prd_list->HSN_CODE;
                    $prd_sale_price = $prd_list->AKS_PRD_PRICE;
                    ...
                    $sale_price = floatval($wgt_chk * $prd_sale_price) / $unit_price;

                    $row = '<tr  name="count"><td>'.($i+1).'<input type="hidden" name="item_count_hidden[]"  id="item_count_hidden'.$i.'" value="'.($i+1).'"></td>
                                <td class="table_tool"> <span class="fs-7 fw-semibold">'.$web_prd_name.'</span></td>
                                <td> <span class="fs-7 fw-semibold">'.$wgt_chk.'</span></td>
                                <td class="table_tool">
                                    <div class="d-flex mb-1">
                                        <div class="d-flex align-items-center">
                                            <div class="symbol symbol-35px me-1">
                                 ...
                                 <div><input type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, \'\').replace(/(\..*)\./g, \'$1\');"  name="sale_wgt[]" id="prd_wgt'.$prd_id.'" value="'.$wgt_chk.'"  onkeyup="price_update('.$prd_id.')" class="form-control form-control-lg form-control-solid fs-7 product_validation"></div></td>
                                 ...
                                 <td id="lbl_price_tot'.$prd_id.'" align="center">'.$sale_price.'</td>
    ...
    public function web_update(){
        ...
        $sale_wgt = $this->input->post('sale_wgt');


        $sale_list = $this->db->query(" SELECT * FROM AKS_WEB_SALE_LIST  WHERE web_id ='".$sale_id."' ")->row();
        $products_info  = json_decode($sale_list->products);   
        $prd_chk = 0;
        $product_data =[];
        $row ="";
        $rows='';
        $array='';
        if(isset($products_info)){
            foreach ($products_info as $i => $pl) {     

                $product_data[]= (object)   [
                                              'order_item_id'       => $pl->order_item_id, 
                                              'product_id'          => $pl->product_id,
                                              'product_weight'      => $sale_wgt[$i],
                                              'product_name'        => $pl->product_name , 
                                              'product_quantity'    => $pl->product_quantity,     
                                              ...
                                              'product_min_price'   => $pl->product_min_price,
                                              'product_max_price'   => $pl->product_max_price
                                            ];
            }
        ...
        $result = $this->db->query("UPDATE AKS_WEB_SALE_LIST SET products='".json_encode($product_data)."' WHERE web_id ='".$sale_id."' ");
           
        if($result)
        {
            add_notification(date('Y-m-d H:i:s'), '', "Others", "karupatti<br>Update Website Sale", $sale_id. ' created By ' .$_SESSION['username'],'', $_SESSION['USERID'], '0', $sale_id);

            $this->session->set_flashdata('g_success',  $sale_id.' Website Sale have been Updated successfully...!!');
        }else{
            $this->session->set_flashdata('g_err', 'Could not update Website sale. !!');
        }
