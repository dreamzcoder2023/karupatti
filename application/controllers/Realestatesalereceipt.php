...
/* ************************************************************************************
        Purpose : To handle all Realestate_sale_receipt functions 2022-08-19
***************************************************************************************/
class Realestatesalereceipt extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model(array("Realestate_sale_receipt_model"));
        ...
        $this->session->set_userdata('comtitle', 'Realestate Sale Receipt');
    }
...

    {
                        ...
                        if($data['from_date_fillter']!=''){
                            ...
                            if($data['to_date_fillter']!=''){
                               ...
                               $data['bill_id'] = $this->input->post('bill_id');
                                 if($data['bill_id']!=''){
                                 $sale_id =" AND property_id LIKE'%".$data['bill_id']."%'";
                                 }
                                  else{
                                 $sale_id ='';
                                }
        ...
        // $data['re_pur_receipt_list']  = $this->db->query("SELECT * FROM realestate_sale_receipt WHERE status='Y' $fdate $tdate $sale_id  ")->result_array();

        $data['re_sale_receipt_list']  = $this->db->query(" SELECT  * FROM 
        ( SELECT *, ROW_NUMBER() OVER (ORDER BY id DESC) AS sl FROM realestate_sale_receipt WHERE  status='Y' $fdate $tdate $sale_id )N 
         WHERE      sl BETWEEN $offset AND $page_limit ")->result_array();

        $data['count'] = count($this->db->query("SELECT * FROM realestate_sale_receipt WHERE status='Y' $fdate $tdate $sale_id ORDER BY id DESC")->result_array());
        $data['exportresalereceipt'] = json_encode($this->db->query("SELECT * FROM realestate_sale_receipt WHERE status='Y' $fdate $tdate $sale_id ORDER BY id DESC")->result_array());

        
        
        $this->load->view('realestate_sale_receipts/realestate_sale_receipts_list',$data);
    }

    public function new_re_prop_sale_receipt()
    {
      $this->load->view('realestate_sale_receipts/new_re_prop_sale_receipt');
    }
    
    public function realestate_sale_receipt_view(){

       $sid = $_GET['id'];
        $data = $this->Realestate_sale_receipt_model->get_pr_list_view($sid);
   ...
   public function payment_details(){
      ...
      $getid = $this->db->query("SELECT * FROM realestate_sale_receipt where id='".$prid."' ")->row();
      // print_r($getid ); exit;
      $payment_id = $getid->recepit_id;

      $data['payment_detail'] = $this->Realestate_sale_receipt_model->get_payment_details($payment_id);

     
  

         $cash = $this->Realestate_sale_receipt_model->get_cash($payment_id);
         // print_r($cash);exit;
         ...
         // print_r($cash_detail);exit;
          ...
          $cheque = $this->Realestate_sale_receipt_model->get_cheque($payment_id);
         // print_r($cheque);exit();
          $cheque_amount    =$cheque->amount;
          $chq_refno        =$cheque->cheque_ref_no;
          $chq_bank     =$cheque->party_bank;
          $chq_detail       =$cheque->remarks;


         

           $upi = $this->Realestate_sale_receipt_model->get_upi($payment_id);
           // print_r($upi);exit;
           ...
           $cash_tr='
            ...
            $cheque_tr='
                    ...
                    <td><label class="col-form-label fw-bold fs-7">'.$chq_bank.'</label></td>
            ...
            $upi_pay=
                     '<tr><td><label class="col-form-label fw-bold fs-3">UPI</label></td>
                      ...
                      <td><label class="col-form-label fw-bold">-</label></td>
    ...
    public function idList()
    {
      ...
      $search =  $_GET['query']; 
      // print_r($search);exit();
      $rows = $this->db->query("SELECT TOP 30 * from realestate_sale where sale_id LIKE '".$search.'%'."' AND balance_amount!='0' AND status='Y' ")->result_array();
      ...
      $res='[';
        if(count($rows)>0) {
          ...
          foreach($rows as $row )
          {
            ...
            $pro_amount = $row['total_property_amount'];
            ...
            $res.='{ "title":"'.$title.'","id": "'.$id.'","sale_id":"'.$sale_id.'","property_id":"'.$property_id.'","date":"'.$date.'","party_name":"'.$party_name.'","ref_name":"'.$ref_name.'","property_type":"'.$property_type.'","ploat_areano":"'.$ploat_areano.'","ploat_areatype":"'.$ploat_areatype.'","price_per_ploat":"'.$price_per_ploat.'","address":"'.$address.'","lattitude":"'.$lattitude.'","longtitude":"'.$longtitude.'","description":"'.$description.'","pro_amount":"'.$pro_amount.'","vao_group":"'.$vao_group.'","register_office":"'.$register_office.'","servey_no":"'.$servey_no.'","partental_dno":"'.$partental_dno.'","patta_no":"'.$patta_no.'","ploat_no":"'.$ploat_no.'","ploat_type":"'.$ploat_type.'","paid_amount":"'.$paid_amount.'","balance_amount":"'.$balance_amount.'","total_property_amount":"'.$total_property_amount.'"},';
      ...
      public function sale_receipt_save()
      {
      ...
      $data['rec_date']  = $this->input->post('recipt_date');
        ...
        // print_r($rec_date);
         $last_pid_detail = $this->Realestate_sale_receipt_model->last_p_r_id_details();
         ...
         if($last_pid_detail){
            ...
            $last_data= $last_pid_detail? $last_pid_detail->recepit_id :0;
                ...
                $request =  request_num(((int)$result+1), 3, "ARESR-");
              ...
              // print_r($recepit_id);
                ...
                $data['sale_id_rec'] =  $this->input->post('sale_id_rec');
        ...
        // print_r($cash);exit;
          $data['pay_mode'] =$this->input->post('bank_chk');
           ...
           $get_last_paid_amt = $this->db->query("SELECT * FROM realestate_sale where sale_id='".$data['sale_id_rec']."' ")->row();
               $old_paid_amt = $get_last_paid_amt->paid_amount;
               $paying_amt   = $data['cr_paid'];
               $update_amt   = $old_paid_amt + $paying_amt;


         $res= $this->db->query("UPDATE realestate_sale SET balance_amount='".$data['cr_bal']."',paid_amount='".$update_amt."'  where sale_id='".$data['sale_id_rec']."'");
               
         $result  = $this->db->query("INSERT INTO realestate_sale_receipt
           (recepit_id
           ,property_id
           ,receipt_date
           ,net_amount
     ...
     VALUES
           ...
           if ($result) {
            ...
            $cash   = $this->Realestate_sale_receipt_model->cashsave($data);
            $cheque = $this->Realestate_sale_receipt_model->chequesave($data);
            $upi    = $this->Realestate_sale_receipt_model->upisave($data);
        ...
        if($result)
        {
            $this->session->set_flashdata('g_success',  $data['recepit_id'].' Property Sale Receipt have been added successfully...');
        }else{
           $this->session->set_flashdata('g_err', 'Could not add Sale !');
        }
        redirect('Realestatesalereceipt/');
