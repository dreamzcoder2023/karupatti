...
public function upcashsave($data)
    ...
    {
      // print_r($this->other_db); exit;
        $this->other_db->reconnect();
        $result  = $this->other_db->query("SELECT * FROM ACC_LEDGERS WHERE GROUP_NAME='Suppliers-karupatti' AND Status='Y'" );
        save_query_in_log();
        $this->other_db->close();
        return $result;
    }
    ...
    public function get_web_sale_list_fill($fdate,$tdate,$party_name,$offset,$page_limit)
    {
      ...
      // $result  = $this->db->query("SELECT * FROM AKS_SALE_ENTRY WHERE status='Y' $fdate $tdate $party_name $sale_id order by sid desc")->result_array();
        ...
        $result = $this->db->query(" SELECT  * FROM  ( SELECT *, ROW_NUMBER() OVER (ORDER BY date DESC) AS sl FROM AKS_WEB_SALE_LIST WHERE  convert_status = 0  $fdate $tdate $party_name ) N  WHERE  sl BETWEEN $offset AND $page_limit ")->result_array();
