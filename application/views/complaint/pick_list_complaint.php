...
<style>
		.dataTables_scroll
    {
        ...
        overflow: auto;
        /*min-height: 257px;*/
        max-height: 157px;/*the maximum height you want to achieve*/
        width: 100%;
    }
											...
											<div class="row">
												<div class="col-lg-8">
													<div class="row">
														...
														<div class="col-lg-4">
														<?php 
																$getcountactive =$this->db->query("SELECT count(pickcomplaint_UID) as idcount FROM PICK_COMPLAINTREPORTS where status=1")->result_array();

																$getinactivecountactive =$this->db->query("SELECT count(pickcomplaint_UID) as inactiveidcount FROM PICK_COMPLAINTREPORTS where status!=1")->result_array();
											...
											<table class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2" id="kt_datatable_raise_complaint">
												<thead>
													<tr class="text-start text-muted fw-bold fs-7 gs-0">
														<th class="min-w-50px" style="display:none;">
												...
												<tbody class="text-gray-600 fw-semibold">
													<?php
														foreach($picklist_complaintdata as $picklistcomplaint)
														{
															//print_r($picklistcomplaint);
															$companyname    = $this->db->query("SELECT COMPANYNAME FROM COMPANY where COMPANYCODE='".$picklistcomplaint['companynameval']."'")->row();
													...
													?>
														<tr>
															...
															<td><?php
																	 echo date("d-m-Y", strtotime($picklistcomplaint['pickcomplaint_date']));
																?></td>
															<td>
																...
																<span><?php echo  $picklistcomplaint['pickcomplaintid']; ?></span>
															</td>
															<td class="ple1"><?php echo $companynameval->COMPANYNAME; ?></td>
															<td class="ple1"><?php echo  $picklistcomplaint['pickcomplaintreason']; ?></td>
															<?php
															$getpicklistcomplaintupdatecount = $this->db->query("SELECT count(pickcomplaint_UID) as countnoupdate from PICKCOMPLAINT_REMARKSUPDATE where pickcomplaint_UID='".$picklistcomplaint['pickcomplaint_UID']."'")->result_array();
															//print_r($getpicklistcomplaintupdatecount[0]['countnoupdate']);exit;
															?>
															<td>
																<label class="badge badge-info"><?php echo isset($getpicklistcomplaintupdatecount[0]['countnoupdate']) ? $getpicklistcomplaintupdatecount[0]['countnoupdate'] : "0"; ?></label>
															</td>
															<td><?php echo  $picklistcomplaint['pickcomplaint_by']; ?></td>
															<td><?php echo  $picklistcomplaint['resolved_by']; ?></td>
															<?php
																if($picklistcomplaint['status']==1)
																{
															...
															<td>
																<span class="text-end">
																	...
																	data-bs-toggle="modal" data-bs-target="#kt_modal_view_picklist_complaint" title="View"
																	onclick="viewpickcomplaintfunc('<?php echo $picklistcomplaint['pickcomplaint_UID']; ?>');">
...
<button class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
																	...
																	<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-250px py-3" data-kt-menu="true" style="">
																		<div class="menu-item px-3">
																			<a href="javascript:;" onclick="updateremarksfunc('<?php echo  $picklistcomplaint['pickcomplaint_UID']; ?>');" class="menu-link px-3">Update Remarks</a>
																		</div>
																		<div class="menu-item px-3">
																			<a href="javascript:;"  onclick="resolvepickcomplaintfunction('<?php echo  $picklistcomplaint['pickcomplaint_UID']; ?>');" class="menu-link px-3">Resolve</a>
																		</div>
																	</div>
...
<button class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" style="display:none;">
																		<i class="fas fa-ellipsis-v eyc"></i>
																	</button>
																	<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-250px py-3" data-kt-menu="true" style="">
																		<div class="menu-item px-3">
...
<?php
								...
								<div class="px-4" style="border-radius: 10px;padding-bottom: 2px;box-shadow: 0 5px 10px #00002947;">
									<div class="row">
										...
										<div class="col-lg-3 d-flex align-items-center">
											<label class="badge badge-info col-form-label fw-bold fs-5" id="nos_updation">-</label>	
										</div>
									</div>
								</div>
							</div>
						...
						<div class="row mt-4">
							...
							<div class="col-lg-6">
								<table class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2" id="kt_datatable_raise_complaint_view">
									...
									<tbody class="text-gray-600 fw-semibold" id="picklistremarklist">
		...
		<div class="modal " id="kt_modal_picklist_resolve_complaint" tabindex="-1" aria-hidden="true" data-bs-keyboard="false" data-bs-backdrop="static" style="background-color: #00000096;">
			<div class="modal-dialog modal-md">
				...
				<div class="modal-content rounded">
					...
					<div class="modal-header justify-content-end border-0 pb-0">
						<!--begin::Close-->
						<div class="btn btn-sm btn-icon btn-active-color-primary" onclick="closeresolvepopupfunc();">
							<!--begin::Svg Icon | path: icons/duotune/arrows/arr061.svg-->
							<span class="svg-icon svg-icon-1">
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
									<rect opacity="0.5" x="6" y="17.3137" width="16" height="2" rx="1" transform="rotate(-45 6 17.3137)" fill="currentColor" />
					...
					<div class="modal-body pt-0 pb-6 px-6 px-xl-10">
						...
						<div class="row mt-3">
							<div class="col-lg-12 fv-row">
								...
								<textarea class="form-control form-control-solid" data-kt-autosize="true" rows="4" name="resolvepickcomplaintremarks" id="resolvepickcomplaintremarks"></textarea>
								<div class="fv-plugins-message-container invalid-feedback"></div>
							</div>
						</div>
						<div class="d-flex align-items-center justify-content-center mt-4">
							<button type="button" class="btn btn-secondary me-3"  onclick="closeresolvepopupfunc();">Cancel</button>
							<button type="button" class="btn btn-primary" onclick="resolvepickcomplaintfunc();">Resolve</button>
						</div>
					</div>
				</div>
				<!--end::Modal dialog-->
		...
		<div class="modal " id="kt_modal_update_remarks" tabindex="-1" aria-hidden="true" data-bs-keyboard="false" data-bs-backdrop="static" style="background-color: #00000096;">
			<div class="modal-dialog modal-md">
				...
				<div class="modal-content rounded">
					...
					<div class="modal-header justify-content-end border-0 pb-0">
						<!--begin::Close-->
						<div class="btn btn-sm btn-icon btn-active-color-primary" onclick="cloasepickcmplaintremarksfunc();">
							<!--begin::Svg Icon | path: icons/duotune/arrows/arr061.svg-->
							<span class="svg-icon svg-icon-1">
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
									<rect opacity="0.5" x="6" y="17.3137" width="16" height="2" rx="1" transform="rotate(-45 6 17.3137)" fill="currentColor" />
					...
					<div class="modal-body pt-0 pb-6 px-6 px-xl-10">
						...
						<div class="row mt-3">
							<div class="col-lg-12 fv-row">
								<label class="fw-semibold required fs-6">Remarks</label>
								<textarea class="form-control form-control-solid" data-kt-autosize="true" rows="4" name="pickcomplaintremarks" id="pickcomplaintremarks"></textarea>
								<div class="fv-plugins-message-container invalid-feedback"></div>
							</div>
						</div>
						<div class="d-flex align-items-center justify-content-center mt-4">
							<button type="button" class="btn btn-secondary me-3"  onclick="cloasepickcmplaintremarksfunc();">Cancel</button>
							<button type="button" class="btn btn-primary" onclick="updateremarkspickcomplaint();">Update</button>
						</div>
					</div>
				</div>
				<!--end::Modal dialog-->
		...
		<input type="hidden" name="pickcomplaintidval" id="pickcomplaintidval" value="" />
		<input type="hidden" name="resolveid" id="resolveid" value="" />
		<?php $this->load->view("script");?>
		<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

		<script>
		var baseurl= $("#baseurl").val();
		function updateremarksfunc(id)
		{
			//alert(id);
			$('#kt_modal_update_remarks').show();
			$('#pickcomplaintidval').val(id);

		}
		function cloasepickcmplaintremarksfunc()
		{
			$('#kt_modal_update_remarks').hide();
			loaction.reload();

		}
		function updateremarkspickcomplaint()
		{
			var pickcomplaintremarks = $('#pickcomplaintremarks').val();
			var pickcomplaintidval = $('#pickcomplaintidval').val();

			if(pickcomplaintremarks.trim()=="")
			{
				$('#pickcomplaintremarks').focus();
				$('#pickcomplaintremarks').addClass('error');
			...
			else
			{
				...
				$('#pickcomplaintremarks').removeClass('error');	
				
				var formData = new FormData();
				formData.append('pickcomplaintremarks', pickcomplaintremarks);
				formData.append('pickcomplaintidval', pickcomplaintidval);
				
				$.ajax(
				{
					url: baseurl+'Raisecomplaint_reports/pickcomplaintremarksupdate',
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) 
					{
						...
						if(returnedData.return_code==0)
						{
							Swal.fire({
								title: 'success!',
								text: 'Pick complaint Remarks Added Successfully',
								icon: 'success',
								confirmButtonText: 'Okay'
							})
							location.reload();
						...
						else
						{
							Swal.fire({
								title: 'error!',
								text: 'Pick complaint Added Remarks Not Successfully',
								icon: 'error',
								confirmButtonText: 'Okay'
							})
		...
		<script>
		function resolvepickcomplaintfunction(id)
		{

			$('#kt_modal_picklist_resolve_complaint').show();
			$('#resolveid').val(id);
		}
		function closeresolvepopupfunc(){

			$('#kt_modal_picklist_resolve_complaint').hide();
			location.reload();
		}
		function resolvepickcomplaintfunc()
		{
			var resolvepickcomplaintremarks =$('#resolvepickcomplaintremarks').val();
			var resolveid =$('#resolveid').val();

			if(resolvepickcomplaintremarks.trim()=="")
			{

				$('#resolvepickcomplaintremarks').focus();
				$('#resolvepickcomplaintremarks').addClass('error');

				Swal.fire({
					title: 'error!',
					text: 'Please Enter Your Remarks resolve..!',
					icon: 'error',
					confirmButtonText: 'Okay'
				})
				return false;
			...
			else
			{
				...
				$('#resolvepickcomplaintremarks').removeClass('error');	
				
				var formData = new FormData();
				formData.append('resolvepickcomplaintremarks', resolvepickcomplaintremarks);
				formData.append('resolveid', resolveid);


				$.ajax(
				{
					url: baseurl+'Raisecomplaint_reports/pickcomplaintremarksresolved',
					type: 'POST',
					data: formData,
					async: false,
					cache: false,
					...
					success: function (response) 
					{
						...
						if(returnedData.return_code==0)
						{
							Swal.fire({
								title: 'success!',
								text: 'Pick complaint Resolved  Successfully',
								icon: 'success',
								confirmButtonText: 'Okay'
							})
							location.reload();
						...
						else
						{
							Swal.fire({
								title: 'error!',
								text: 'Pick complaint  Resolved Not Successfully',
								icon: 'error',
								confirmButtonText: 'Okay'
							})
		...
		function viewpickcomplaintfunc(id)
		{

			$('#kt_modal_view_picklist_complaint').show();
			
			var picklistcomplaintID = id;

			var formData = new FormData();
			formData.append('picklistcomplaintID', picklistcomplaintID);
			

			$.ajax(
			{
				url: baseurl+'Raisecomplaint_reports/viewpicklistcomplaint',
				type: 'POST',
				data: formData,
				async: false,
				cache: false,
				...
				success: function(response) 
				{
					...
					var returnedData = JSON.parse(response);
					console.log(returnedData.picklistcomplaintdata);

				

					$('#complaintbillno').html(returnedData.picklistcomplaintdata[0].pickcomplaintid);
				$('#complaint_Date').html(returnedData.picklistcomplaintdata[0].pickcomplaint_date);
				$('#complaintBy').html(returnedData.picklistcomplaintdata[0].pickcomplaint_by);
				$('#complaint_description').html(returnedData.picklistcomplaintdata[0].pickcomplaintreason);
				$('#remarks').html(returnedData.picklistcomplaintdata[0].remarks);
				...
				$('#resolve_Date').html(returnedData.picklistcomplaintdata[0].resolved_date);
				$('#resolvedBy').html(returnedData.picklistcomplaintdata[0].resolved_by);
				if(returnedData.picklistcomplaintdata[0].status==1){
        	...
        	r='<i class="fa-solid fa-star" style="color:#d2d4dc;"></i>&nbsp;-';	
				...
				$('#picklistremarklist').empty().append(returnedData.picklistremarklist);
