  ...
  .dataTables_scroll thead{
   ...
   background-color: #ec9629;
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
											...
											<div class="mb-5 hover-scroll-x">
													<div class="d-grid">
														<ul class="nav nav-tabs flex-nowrap text-nowrap">
															<li class="nav-item">
																<a class="nav-link active btn btn-active-light btn-color-black-800 btn-active-color-primary rounded-bottom-0 fw-bold fs-6" data-bs-toggle="tab" href="#kt_tab_pane_1">Lock In</a>
															</li>
															<li class="nav-item">
																<a class="nav-link  btn btn-active-light btn-color-black-800 btn-active-color-primary rounded-bottom-0 fw-bold fs-6"  href="<?php echo base_url(); ?>Lock_in_lock_out/history">History</a>
												...
												<div class="tab-content" id="myTabContent">
													<div class="tab-pane fade show active" id="kt_tab_pane_1" role="tabpanel">
														...
														<form action="<?php echo base_url();  ?>Lock_in_lock_out/lock_in" method="post"  id="frm_lock_in">
															<input type="hidden" name="company_id" id="company_id">
															<input type="hidden" name="li_reason" id="li_reason" value="">
														...
														<div class="row mt-4">
															...
															<div class="d-flex align-items-center justify-content-end">
																<p class="btn btn-primary me-2" onclick="lock_in_validation();">Lock In</p>
															</div>
															</form>
		...
		<!--begin::Modal - Enter Lock In Reason-->
		<div class="modal fade" id="kt_modal_lockin_reason" tabindex="-1" aria-hidden="true">
			<!--begin::Modal dialog-->
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body">
						<div class="mb-1 text-center">
							<!-- <h1>Lock In Reason</h1> -->
							<h2>Reason for Lock In</h2>
						</div>
						<div class="row">
							<label class="col-lg-12 col-form-label fw-semibold fs-6">Lock In Reason</label>
							<!-- <label class="col-lg-12 col-form-label fw-bold fs-6" id="lbl_description">The Jewel Value is maximum 15,000.00</label> -->
							<select id="lock_in_reason" name="lock_in_reason" class="form-select form-select-solid "  onchange="save_reason_form_input()">
								<option value="">Select Reason</option>
								<option value="New Loan">New Loan</option>
								<option value="From BJ">From BJ </option>
								<option value="From Renewal">From Renewal</option>

							</select>
							<div class="fv-plugins-message-container invalid-feedback" id="lock_in_reason_err"></div>
						</div>
		...
		<!--end::Modal -Enter Lock In Reason-->
...
<input type="hidden" id="baseurl" value="<?php echo base_url(); ?>">
    ...
    $(this).closest('tr').remove();
		...
		<script >
			...
			{
					var li_reason=$('#lock_in_reason').val();
					// alert(li_reason);
					$('#li_reason').val(li_reason);
			}
		</script>
		<script >
				...
				$('#reason_confirm').click(function() {
					...
					if(li_reason=='')
					{
							document.getElementById('lock_in_reason').focus();
							$('#lock_in_reason_err').html('Enter Lock In Reason');
							// document.getElementById("pop_up_error").click();
					}
					else
					{
							// lock_in_validation();
							$('#kt_modal_lockin_reason').removeClass('show');
							$('.modal-backdrop').removeClass('show');

							$('#kt_modal_lockin_reason').hide();
				   		$('.modal-backdrop').hide();
			...
			function lock_in_validation() 
			{
				...
				{
						// alert("Enter items to Lock in");
						$('#err_msg').html('Enter items to Lock in');
							document.getElementById("pop_up_error").click();
						document.getElementById('scan_item').focus();
						err++;
				}
				if(err>0){return false;}else{
					$('#kt_modal_lockin_reason').addClass('show');
							$('.modal-backdrop').addClass('show');

							$('#kt_modal_lockin_reason').show();
				   		$('.modal-backdrop').show();
				}
			}
								...
								if(dup==0)
								{
									...
									success: function(response)
									{
										...
										if(response.trim()=='0')
										{
											...
											$('#scan_item').val('');
												document.getElementById('scan_item').focus();
												$('#err_msg').html('Loan Status is in Waiting for Approval');
												document.getElementById("pop_up_error").click();
										}
										else
										{
