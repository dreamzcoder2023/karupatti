<?php $this->load->view("common");?>
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Lot List
									<!--begin::Separator-->
									<span class="h-20px border-gray-400 border-start ms-3 mx-2"></span>
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										<span>Company - </span>
										<span><?php if($company_fill==''){ echo "All"; } else{  
											 				$company  = $this->db->query("SELECT * FROM COMPANY WHERE ACTIVE='Y' AND COMPANYCODE='". $company_fill."' ")->row();
														 echo $company?$company->COMPANYNAME:'-'; } ?></span>
									</label>
									<span class="h-20px border-gray-400 border-start ms-3 mx-2"></span>
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										<span>Supplier - </span>
										<span> <?php if($supplier_name==''){ echo "All"; }else{ echo $supplier_name?$supplier_name:'-'; } ?></span>
									</label>
									<span class="h-20px border-gray-400 border-start ms-3 mx-2"></span>
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										<span>Lot No - </span>
										<span> <?php if($dt_fill_no==''){ echo "All"; }else{ echo $dt_fill_no; } ?></span>
									</label>
									<span class="h-20px border-gray-400 border-start ms-3 mx-2"></span>
									<label class="d-flex align-items-center text-primary fw-bold my-1 fs-5">
										<span>Date - </span>
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
										...
										<div class="card-body py-4">
											<div class="row">
												<div class="col-lg-8">
													<div class="row">
														<label class="col-lg-4 form-label text-center fs-4 fw-bold">Total Count</label>
														<label class="col-lg-4 form-label text-center fs-4 fw-bold">Total Weight</label> 
													</div>
													<div class="row">
														<label class="col-lg-4 text-success text-center fs-2 fw-bold"><?php if($Lotentry_list1->count==''){ echo "0"; } else{ echo $Lotentry_list1->count; } ?></label>
														<label class="col-lg-4 text-success text-center fs-2 fw-bold"><?php if($Lotentry_list1->weight==''){ echo "0"; } else{ echo $Lotentry_list1->weight; } ?></label>
														<!-- <label class="col-lg-4 text-success text-center fs-2 fw-bold">	<?php //if($Lotentry_list1->amount==''){ echo "0"; } else{ echo IND_money_format($Lotentry_list1->amount); }?></label> -->
													</div>
												</div>
												<div class="col-lg-4">
													<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
														<!--begin::Filter-->
														<button type="button" class="btn btn-sm btn-outline btn-outline-solid btn-outline-warning btn-active-light-primary me-3" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Filter</button> 
														<div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
															<div class="px-7 py-5" data-kt-user-table-filter="form">
																<div class="scroll-y mh-325px my-1 px-3">
																	<form method="POST"action="<?php echo base_url(); ?>Lotentry"id="fill_form">
																		<div class="mb-2">
																			<label class="form-label fs-6 fw-semibold">Company</label>
																			<select class="form-select form-select-solid fw-semibold" data-control="select2" data-close-on-select="true"  name="company_fill" id="company_fill">	
																				<option value="" >All</option>
																				<?php foreach($company_list as $company_list){?>
																				<option value="<?php echo $company_list['COMPANYCODE'] ;?>" <?php if($company_list['COMPANYCODE']==$company_fill ){ echo "selected"; } ?>><?php echo $company_list['COMPANYNAME'];?></option> 
																				<?php }?>
																			</select>
																		</div>
																		<div class="mb-2">
																		...
																		<div class="mb-2">
																			...
																			<input class="form-control form-control-solid " name="dt_fill_lot_no" placeholder="Lot Number" id="dt_fill_lot_no" value="<?php echo $dt_fill_no?$dt_fill_no:'' ?>"  />
																		</div>
																	    <div class="mb-2">
														...
														<?php if(isset($_SESSION['Lot entryAdd'])){ if($_SESSION['Lot entryAdd']==1){?>
															<a href="<?php echo base_url(); ?>Lotentry/Lotentry_add">
																...
																</span>New Lot Entry</button>
															</a>
														<?php }} ?> 
													</div>
												</div>											
												<div class="modal fade" id="kt_modal_export_users_lot" tabindex="-1" aria-hidden="true">
													<!--begin::Modal dialog-->
													<div class="modal-dialog modal-dialog-centered mw-650px">
														...
														<div class="modal-content">
															...
															<div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
																<!--begin::Form-->
																<form id="kt_modal_export_users_lot_form" class="form" action="#">
																	<!--begin::Input group-->
											...
											<div class="row mt-4">
											<!--begin::Table-->
											<table  class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2 mt-4" id="kt_lot_list">
												<thead>
												   <tr class="text-start text-muted fw-bold fs-7 gs-0">
													   ...
													   <th class="min-w-80px">Lot No</th>
														...
														<th class="min-w-50px" >Item Type</th>
														<th class="min-w-25px">Count</th>
														<th class="min-w-50px">Tot Wgt</th>
														<th class="min-w-50px">Met Wgt</th>
														<th class="min-w-175px">Status</th>
														<th class="min-w-150px" ><span class="text-end">Actions</span></th>
												   </tr>
												</thead>
												<tbody class="text-gray-600 fw-semibold">
													<?php 

													$i=1; if(isset($Lotentry_list)){ foreach($Lotentry_list as $lotentrylist){?>
													<tr>													
														<td  class="ple1"> 
															<?php 
														...
														<td class="ple1"><?php
														 $company  = $this->db->query("SELECT * FROM COMPANY WHERE ACTIVE='Y' AND COMPANYCODE='". $lotentrylist['company_id']."' ")->row(); 
														 	 echo $company?$company->COMPANYNAME:'-';
														 ?></td>
														<td class="ple1"><?php echo $lotentrylist['lot_no']?$lotentrylist['lot_no']:'-';?></td>
														<td class="ple1">
															<?php	$image_url =  base_url().'lot_img/'. $lotentrylist['img']; 
															    if(@getimagesize($image_url)){ ?>
																	<a class="d-block overlay"  href="<?php echo base_url(); ?>lot_img/<?php echo $lotentrylist['img']; ?> " data-fslightbox="lightbox-basic">
																	    <!--begin::Image-->
																	    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded w-40px h-40px"
																	    style="background-image:url('<?php echo base_url(); ?>lot_img/<?php echo $lotentrylist['img']; ?>')">
																	    </div>
																	    <!--end::Image-->
																	    <!--begin::Action-->
																...
																<?php }else{ ?>
																	 <a href="javascript:;" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm"   >
									            						<div class="image-input" data-kt-image-input="true">
																			<div class="image-input-wrapper w-40px h-40px" style="background-image: url(<?php echo base_url() ?>assets/images/Lot_images.jpg)">
																			</div>
																		</div>
																	</a>
															<?php } ?>
														</td>
														<td class="ple1"><?php echo $lotentrylist['supplier']?$lotentrylist['supplier']:'-';?></td>
														<td class="ple1"> 
															<?php
																if(($lotentrylist['metal_type']==1)||($lotentrylist['metal_type']==2)){
																$item_type=$this->db->query("SELECT * FROM jewel_type WHERE jewel_type_id='".$lotentrylist['metal_type']."'  ")->row();

																echo $item_type->jewel_type ? $item_type->jewel_type:'-';
																}
													 		?> 
													 	</td>
														<td class="ple1"><?php
														$no_items=$this->db->query("SELECT count(lot_id) as count FROM lot_entry_detail WHERE lot_id='".$lotentrylist['lot_id']."'  ")->row();
														echo $no_items->count?$no_items->count:'-';
														?></td>
														<td class="ple1"><?php echo number_format($lotentrylist['item_weight'],3);?></td>
														<td class="ple1"><?php echo number_format($lotentrylist['pure_metal_weight'],3);?></td>
														<!--<td class="ple1"><?php//echo number_format($lotentrylist['amount_to_pay'],2);?></td>
														<td class="ple1"><?php//echo number_format($lotentrylist['balance_amount'],2);?></td>-->
														<td>
															<?php if($lotentrylist['status']=='Y'){ ?>
															...
															<?php } ?>
															<?php if($lotentrylist['status']=='U'){ ?>
															<label class="col-form-label fw-semibold fs-7"><span style="background-color:#46dcf9;border-radius: 8px;padding: 5px 15px 5px 15px;"  >Update Required</span><span><i class="fa-solid fa-circle-info eyc fs-5 ms-1" data-bs-toggle="modal"  data-bs-target="#kt_modal_update_tag_entry_view"  onclick="lot_edit_view('<?php echo $lotentrylist['lot_id']; ?>')" title="View Update Update Required"></i></span>
															</label>
															<?php } ?> 
															<?php if($lotentrylist['status']=='A'){ ?>
															<label class="col-form-label fw-semibold fs-7"><span style="background-color:#50cd89;border-radius: 8px;padding: 5px 15px 5px 15px;">Approved</span>
															<?php } ?>
														</td>
														<td>
															<?php if(isset($_SESSION['Lot entryView'])){ if($_SESSION['Lot entryView']==1){?>
																<span class="text-end">
																	<a href="<?php echo base_url(); ?>Lotentry/Lotentry_view/<?php echo $lotentrylist['lot_id'];?>" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" target="_blank">
																		<i class="bi bi-eye-fill eyc"></i>
																	</a> 
																	<?php }} ?>
																	<?php if(isset($_SESSION['Lot entryEdit'])){ if($_SESSION['Lot entryEdit']==1){?>
																...
																<?php  if($lotentrylist['status']!='A'){ ?>
																	...
																	<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-200px py-3" data-kt-menu="true" style="">												
																		<div class="menu-item px-3">
																			<a href="#" class="menu-link flex-stack px-3" data-bs-toggle="modal" data-bs-target="#kt_modal_verify_tag_entry" onclick="lot_approve('<?php echo $lotentrylist['lot_id']; ?>')">Approve</a>
																		</div>
																		<?php  if($lotentrylist['status']!='U'){ ?>								
																			<div class="menu-item px-3">
																				<a href="#" class="menu-link px-3" data-bs-toggle="modal" data-bs-target="#kt_modal_update_tag_entry" onclick="lot_edit('<?php echo $lotentrylist['lot_id']; ?>')">Send for Update</a>
																			</div>
																		<?php } ?>
																	</div>
															<?php } ?>
											...
											<form   method="POST" id="filter_form" action="" enctype="multipart/form-data" >	
											    <input type="hidden" id="dt_fill_lot" name="dt_fill_lot"  value="<?php echo $dt_fill; ?>">
											    <input type="hidden" id="from_date_fillter_lot" name="from_date_fillter_lot" value="<?php echo $from_date_fillter; ?>">
				...
				<div class="modal-content rounded">
					...
					<div class="modal-body">
						<div class="d-flex justify-content-center">
							<div class="image-input" data-kt-image-input="true" id="image_show">
				...
				<div class="modal-content rounded">
					<div class="swal2-icon swal2-warning swal2-icon-show" style="display: flex;">
						...
						<div class="swal2-html-container" id="swal2-html-container" style="display: block;">
							...
							<input type="hidden"   id="approve_no" >
					...
					<div class="modal-body pt-0 pb-8 px-5 px-xl-20">
						...
						<div class="row">
							...
							<div class="col-lg-8 fv-row fv-plugins-icon-container">
								...
								<input type="hidden" id="update_no_hide" name="update_no_hide" >
		...
		<div class="modal fade" id="kt_modal_update_tag_entry_view" tabindex="-1" aria-hidden="true">
			<!--begin::Modal dialog-->
			<div class="modal-dialog modal-md">
				<!--begin::Modal content-->
				<div class="modal-content rounded">
					...
					<div class="modal-body pt-0 pb-8 px-5 px-xl-20">
						...
						<div class="row">
							<label class="col-lg-4 col-form-label fw-semibold fs-6">Description</label>
							<div class="col-lg-8 fv-row fv-plugins-icon-container">
								<label class="col-lg-4 col-form-label fw-semibold fs-6 " id="update_description_view" ><?php //echo $Lotentry_list_view->update_description; ?></label>
								<div class="fv-plugins-message-container invalid-feedback"></div>
							</div>
						</div>
						<div class="mt-4 d-flex justify-content-center">
								<button type="reset" class="btn btn-secondary me-2" data-bs-dismiss="modal" onclick="closeViewModal()">Close</button>
	...
	$(document).ready(function() {
        ...
        $(".move_to").on("click", function () {
			value=$(this).val();
			// alert(value);
            $('#filter_form').attr('action', "<?php echo base_url(); ?>lotentry?page="+value);
            $("#filter_form").submit();
            e.preventDefault();
        });
    });
		...
		function lot_approve(val)
			...
			function lot_edit(val)
			{
				...
				success: function(response)
				{
					...
					$('#update_id_hide').val(resp[1]);		
					$('#update_no_hide').val(resp[2]);
			...
			function lot_edit_view(val)
			{
				var baseurl= $("#baseurl").val();
				//alert(val);
				$.ajax({
				type: "POST",
				url: baseurl+'Lotentry/lotentry_update_required_view',
				async: false,
				type: "POST",
				data: "id="+val,
				dataType: "html",
				success: function(response)
				{
					...
					$('#update_description_view').html(response);		
				//$('#kt_modal_update_tag_entry_view').empty().append(response);
				// $('#kt_modal_update_tag_entry').addClass('show');
				//$("#kt_modal_delete_role").css("display", "block");
				}
				});
		...
		function image_view(val)
			{
				...
				$.ajax({
				type: "POST",
				url: baseurl+'Lotentry/image_view',
				async: false,
				type: "POST",
				data: "id="+val,
				dataType: "html",
				...
				// $('#kt_modal_view_jewel_img').addClass('show');
			...
			{
				var baseurl= $("#baseurl").val();
				 $('#kt_modal_view_jewel_img').removeClass('show');
				// $('#kt_modal_view_jewel_img').model('hide');
				$('.modal-backdrop').removeClass('show');
				$('#kt_modal_update_tag_entry').removeClass('show');
				$('.modal-backdrop').removeClass('show');
				$('#kt_modal_verify_tag_entry').removeClass('show');
				$('.modal-backdrop').removeClass('show');
			...
			{ 
				...
				var no  = document.getElementById('approve_no').value;
				// alert(val);
			...
			{ 
				...
				var no  = document.getElementById('update_no_hide').value;
				...
				async: false,
				data:"field="+val+"&update_description="+update_description+"&no="+no,
				success: function(res)
				{
				window.location.href = baseurl+'Lotentry';
				}
