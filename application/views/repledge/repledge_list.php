											...
											<table id="kt_datatable_responsive" class="table align-middle table-row-dashed table-striped table-hover fs-7 gy-1 gs-2">
												<thead>
											   	<tr class="fw-bold fs-7 text-gray-800 px-7">
												    ...
												    <th class="min-w-100px">Company</th>
														...
														<th class="min-w-80px">Remainder</th>
												...
												<tbody class="text-gray-600 fw-semibold">
													...
													<tr>
														...
														<td class="ple1">
														<i class="fa fa-copy" style="    font-size: 9px;cursor: pointer;" onclick="myFunction('<?php echo  $rlist['RP_BILLNO'];?>');"></i>
															&nbsp;<div id="copied-success" class="copied" ><span  style="display:none;">Copied !</span></div></span><span class="k-tooltip-content"><?php echo $rlist['RP_BILLNO']; ?></span>
														...
														<?php 
															...
															?>
																<span class="btn btn-icon btn-circle fs-8 fw-bold shadow text-white mb-1" style="background:blue;width: 20px !important;height: 20px !important;float: right;">MX</span>
															<?php
														}
														...
														<td>
															...
															$remain_days=$this->db->query("SELECT * FROM REPLEDGE_MASTER WHERE RP_BILLNO='".$rlist['RP_BILLNO']."'")->row();
															$date1= date('Y-m-d');
														
															$totmonths =$remain_days->MONTHS;
															$totdays =0;
														
															$date2= $remain_days->ENDATE;

															for($i=0;$i<$totmonths;$i++)
															{

																$months = date('Y-m-d', strtotime('+1 month', strtotime($date2)));
																$date2 = $months;
															...
															$DateDiff = floor( strtotime($date2 ) - strtotime( $date1 ) ) / 86400 ;
															if($DateDiff>=0){

																echo '<span class="badge badge-primary">'.$DateDiff.' Days </span>';
															}
															else{

																		echo '<span class="badge badge-danger">'.$DateDiff.' Days </span>';
															}
		...
		(function()
{
  ...
  for(let i = 0; i < buttons.length; i++)
  {
    ...
    {
      let button = e.target;
      let bnumber = button.parentNode.parentNode.getElementsByClassName('bnumber')[0].innerHTML;
