...
<style>
  ...
  .dataTables_scroll thead{
						...
						<div id="kt_toolbar_container" class="container-fluid d-flex align-items-center">
							...
							<div class="flex-grow-1 flex-shrink-0 me-5">
								...
								<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
									<!--begin::Title-->
									<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">Real Estate - Agent - Total Sum of Amount
									<!--begin::Separator-->
								...
								<div class="col-xxl-8">
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
										...
										<div class="card-body py-4">
											...
											<form method="POST" autocomplete="off" action="<?php echo base_url(); ?>Reports/agenttotalsumamount" enctype="multipart/form-data" >
											
											<div class="accordion" id="kt_accordion_item_list" style="display:none;">
											    <div class="accordion-item">
											        ...
											        <div id="kt_accordion_item_list_body_1" class="accordion-collapse collapse" aria-labelledby="kt_accordion_item_list_header_1" data-bs-parent="#kt_accordion_item_list" style="display:none;">
											            <div class="accordion-body">
											            	<div class="row">
																			<div class="col-lg-3">
																				<div class="row">
																					<label class="col-lg-4 col-form-label fw-semibold fs-6">Agent</label>
																					<div class="col-lg-8">
																						<select class="form-select form-select-solid" data-control="select2"  id="agentName" name="agentName"  id="agent_fill" name="agent_fill" >
																							<option value="ALL" >All</option>
																							<?php foreach($agent_list as $agentval)
																								{ ?>
																									<option value="<?php echo $agentval['LEDGER_NAME']; ?>"  <?php if ($agentval['LEDGER_NAME']==$agent_fill){echo 'selected';} ?>><?php echo $agentval['LEDGER_NAME']; ?></option>
																							<?php } ?>
																						</select>
																					</div>
																				</div>
																		...
																		<div class="row" >
																			<div class="col-lg-6">
																				<div class="row">
																					<div class="col-lg-3 fv-row fv-plugins-icon-container fv-plugins-bootstrap5-row-invalid">
																						<div class="d-flex align-items-center">
																							<label class="form-check form-check-inline form-check-solid me-1 is-invalid">
																								<input class="form-check-input" name="communication[]" type="checkbox" value="1" id="amount_radio" onclick="amt_radio()">
																							</label>
																							<span class="col-form-label fw-semibold fs-6">Amount</span>
																						</div>
																					</div>
																					<div class="col-lg-3" id="amount_radio_tbox" style="display: none;">
																						<select class="form-select form-select-solid" data-control="select2"data-hide-search="true" onchange="amt_tbox()" id="amt_radio_tbox" name="amt_radio_tbox">
																							<option value="=">=</option>
																							<option value=">">></option>
																							<option value="<"><</option>
																							<option value=">=">>=</option>
																					...
																					<div class="col-lg-3" id="amount_radio_tf1" style="display: none;">
																						<input type="text" class="form-control form-control-lg form-control-solid" id="amountval" name="amountval" value="">
											...
											<div class="d-flex align-items-center justify-content-end mt-3" >
												<button type="button" class="btn btn-sm btn-outline btn-outline-solid btn-outline-warning btn-active-light-primary me-3" style="display:none;">Export</button>
												<button type="submit" class="btn btn-sm btn-outline btn-outline-solid btn-outline-warning btn-active-light-primary me-3" style="display:none;">Go</button>
											</div>
										</form>
											<!--begin::Table-->
											<div class="row mt-2">
												<?php// print_r($Agent_totnumamount_list); ?>
												<table class="table align-middle table-row-dashed table-striped fs-7 gy-2 gs-2" id="kt_datatable_re_agent_total_amt_report">
													
													<thead>
					    							<tr class="text-muted fw-bold fs-7 gs-0 align-items-center">
							            		<th class="min-w-50px">S.No</th>
							            		<th class="min-w-100px">Date</th>
							            		<th class="min-w-80px">Property No</th>
							            		<th class="min-w-80px">Agent</th>
											        <th class="min-w-80px">Land Name</th>
							            		<th class="min-w-50px">Party Info</th>
											        <th class="min-w-50px">Plot Area<br>(No / Type)</th>
											        <th class="min-w-150px">Land Amt</th>
							            		<th class="min-w-150px">Agent Amt</th>
							            		<th class="min-w-80px"><span class="text-end">Actions</span></th>
													...
													<tbody class="text-gray-600 fw-semibold">
														<?php
														$i=1;
														foreach($Agent_totnumamount_list as $angetallvalue){ ?>
														<tr>
															<td><?php echo $i; ?></td>
									            <td>
									            	<?php
          												$date=date('d-m-Y',strtotime($angetallvalue['pur_date']?$angetallvalue['pur_date']:$angetallvalue['sale_date']));
                                  ...
                                  echo date("$format", strtotime($date));
					                      ?>
									            ...
									            <td>
										            	<?php if ($angetallvalue['ag_ref']=='Realestate Purchase') { ?>


										            		<?php if ($angetallvalue['pur_status']=='Existing'){ echo '<span class="badge badge-info badge-circle fs-6 badge-sm mt-1"title="Exiting Property">EP</span>';}
										            				else{echo '<span class="badge badge-primary badge-circle fs-6 badge-sm mt-1"title="Exiting Property">PP</span>';} ?>

										            			<?php  echo $angetallvalue['ag_prop']; ?>

										            	<?php }else{ ?>
										            			<span class="badge badge-success badge-circle fs-6 badge-sm mt-1">S</span> <?php  echo $angetallvalue['ag_prop']; ?>
										            	<?php } ?>
									            </td>
									            <td class="ple1"><?php echo $angetallvalue['agent_name']; ?></td>
									            <td class="ple1"><?php echo $angetallvalue['pur_land_name']?$angetallvalue['pur_land_name']:$angetallvalue['sale_land_name']; ?></td>
									            <td class="ple1">
																<span>
																	<?php $rating=$angetallvalue['pur_party_rat']?$angetallvalue['pur_party_rat']:$angetallvalue['sale_party_rat'];?>
																	 <i class="fa-solid fa-star fs-7" style="
															        <?php															          		
																	          if($rating ==3) echo 'color:#50cd89;';
																						if($rating ==2) echo 'color:#ffc700;';
																						if($rating ==1) echo 'color:red;';
																						if($rating =='') echo 'color:#d2d4dc;';
																			?>">
																	</i>
																</span>
																<span class="align-items-center"><?php echo $angetallvalue['pur_party_name']?$angetallvalue['pur_party_name']:$angetallvalue['sale_party_name']; ?></span>
															</td>
															<td class="ple1">
																<label class="fw-bold">
																	<i class="fas fa-map-marker-alt fs-6"></i>&nbsp;
																</label>
																<label><?php echo $angetallvalue['pur_plno']?$angetallvalue['pur_plno']:$angetallvalue['sale_plno']; ?></label>
																<label> / </label>
																<label><?php echo $angetallvalue['pur_type']?$angetallvalue['pur_type']:$angetallvalue['sale_type']; ?></label>
															</td>
									            <td><?php $land_amt = $angetallvalue['pur_tot_amt']?$angetallvalue['pur_tot_amt']:$angetallvalue['sale_tot_amt']; echo number_format($land_amt?$land_amt:0,2,'.',','); ?></td>
									            <td><?php echo number_format($angetallvalue['agent_amt_give']?$angetallvalue['agent_amt_give']:0,2,'.',','); ?></td>
									            <td>
									            	<?php if ($angetallvalue['ag_ref']=='Realestate Purchase') {  ?>

									            	<?php if ($angetallvalue['pur_status']=='Existing'){?>
									            					<span class="text-end">
									            						<a href="<?php echo base_url();?>Realestateproperty/realestate_property_view/<?php echo $angetallvalue['pur_id']?$angetallvalue['pur_id']:$angetallvalue['sale_id']; ?>" target="_blank" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
																							<i class="bi bi-eye-fill eyc fs-3"></i>
																						</a>
									            					</span>
									            	<?php } else{ ?>
										            					<span class="text-end">
																						<a href="<?php echo base_url();?>Realestatepurchase/realestate_view_purchase/<?php echo $angetallvalue['pur_id']?$angetallvalue['pur_id']:$angetallvalue['sale_id']; ?>" target="_blank" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
																...
																<?php }else{ ?>
																<span class="text-end">
																	<a href="<?php echo base_url(); ?>Realestatesale/sales_view/<?php echo $angetallvalue['pur_id']?$angetallvalue['pur_id']:$angetallvalue['sale_id']; ?>" target="_blank" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
	...
	<?php $this->load->view("script");?>
		...
		<script>
		$('#kt_datatable_re_agent_total_amt_report').DataTable( {
		"aaSorting": [],
        dom: 'Bfrtip',
        buttons: [
            								{
		                            extend: 'copyHtml5',
		                            title: 'Real Estate - Agent - Total Sum of Amount',
		...
		<script>
			      $("#kt_datatable_re_agent_total_amt_report").kendoTooltip({
			        filter: "td",
			        show: function(e){
