  ...
  /*Auto complete css start*/
	...
	<body data-kt-name="metronic" id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed">
							...
							<div class="row gy-5 g-xl-8">
								...
								<div class="col-xxl-8">
									...
									<div class="card card-xxl-stretch mb-5 mb-xl-8">
										<form name="sale_deliverymode_form" id="sale_order_form" method="POST" action="<?php echo base_url(); ?>Sales_quotation/sales_quotation_update/<?php echo $quotation->ID; ?>" enctype="multipart/form-data"  onsubmit="return sale_order_validation()">	
											<!--begin::Card body-->
											<div class="card-body py-4">
														...
														<div class="row">
															...
															<div class="col-lg-4 fv-row">
																<div class="d-flex align-items-center">
																	...
																	<!--end::Svg Icon-->
																	<input class="form-control form-control-solid ps-12" name="date" placeholder="Date" id="quotation_date" value="<?php echo date("d-m-Y", strtotime($quotation->QUOT_DATE));  ?>" />
																</div>
															...
															<div class="col-lg-4 fv-row fv-plugins-icon-container">
																<select class="form-select form-select-solid" name="quot_type" id="quot_type" data-control="select2" data-hide-search="true" onchange="quotation_type();">
																	<option value="party" <?php echo $quotation->QUOT_TYPE == "party" ? "selected":''; ?>>Party</option>
																	<option value="supplier" <?php echo $quotation->QUOT_TYPE == "supplier" ? "selected":''; ?>>Supplier</option>
																</select>
															</div>

															<?php if($quotation->QUOT_TYPE == 'party'){ ?>
																...
																<div class="col-lg-10 fv-row" id="quot_type_party_box">
																	<input type="text" name="sale_party" id="sale_party" class="form-control form-control-lg form-control-solid me-3" placeholder="Select Party Name" value="<?php echo $quotation->QUOT_TYPE == "party" ? $quotation->NAME:''; ?>"  oninput="first_nm_party();" >
																		<div class="fv-plugins-message-container invalid-feedback" name="party_err" id="party_err"></div>
																		<input type="hidden" name="party_id_hidden" id="party_id_hidden" value="<?php echo $quotation->PARTY_ID; ?>">
																	<div class="fv-plugins-message-container invalid-feedback" id="party_name_err"></div>
																</div>
															<?php }else{ ?>
																<label class="col-lg-2 col-form-label fw-semibold fs-6 required" id="quot_type_supplier" style="display: block;">Supplier</label>
																<div class="col-lg-10 fv-row" id="quot_type_supplier_box" style="display:block;">
																	<select class="form-select form-select-solid" name="supplier_id" id="supplier_id_name" data-control="select2" data-hide-search="false" onchange="supilerget();">
																		...
																		<?php foreach ($suppliers_lists as $sllist) { ?>		
																				<option value="<?php echo $sllist['LEDGER_SNO']; ?>" <?php echo $sllist['LEDGER_SNO']==$quotation->SUPPLIER_ID ? "selected":""; ?>  >
													...
													<div class="accordion-item">
														<h2 class="accordion-header" id="kt_accordion_sales_quotation_header_2">
															<button class="accordion-button fw-bold fs-6 collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#kt_accordion_sales_quotation_body_2" aria-expanded="true" aria-controls="kt_accordion_sales_quotation_body_2">
																...
																<span id="selected_count"><?php echo count($selected_values);?></span>
																<span class="me-4">&emsp; - &emsp;Total Amount&emsp; -</span>
																<span id="selected_amount"><?php echo array_sum($selected_amount);?></span>
															</button>
														</h2>
																...
																<table class="table align-middle table-row-dashed table-striped fs-7 gy-1 gs-2 dataTable" id="add_quotation_table">
																	...
																	<tbody class="text-gray-600 fw-bold fs-6">
																		...
																		<?php for($i=1;$i <= 20;$i++){ 
																			...
																			if(isset($quotation->TAG_ITEMS[$key])){ ?>
																				...
																				<tr id="row_<?php echo $i; ?>">
																						...
																						<select class="form-select form-select-solid fs-7 select_item" data-control="select2" data-hide-search="false" data-width="150px" name="item_details[]"  data-index="<?php echo $i; ?>" id="select_tag_<?php echo $i; ?>"> 	
																							...
																							<?php foreach($tag_list as $tag){  
																								...
																								<option value="<?php echo $tag['tag_id']; ?>"
																										<?php echo $tag['tag_id'] ==  $quotation->TAG_ITEMS[$key]->tag_id ? "selected":"";?>
																								><?php echo $tag['tag_id']; ?></option>
																							<?php }} ?>
																					...
																					<td id="item_type_<?php echo $i; ?>"><?php echo $tag['metal_type']; ?></td>
																					<td id="item_name_<?php echo $i; ?>"><?php echo $tag['item_name']; ?></td>
																					<td id="item_quality_<?php echo $i; ?>"><?php echo $tag['quality']; ?></td>
																					<td id="item_purity_<?php echo $i; ?>"><?php echo $tag['purity']; ?></td>
																					<td id="item_weight_<?php echo $i; ?>"><?php echo $tag['weight']; ?></td>
																					<td id="item_stone_weight_<?php echo $i; ?>"><?php echo $tag['stone']; ?></td>
																					<td id="item_net_weight_<?php echo $i; ?>"><?php echo $tag['net_weight']; ?></td>
																					...
																					<td id="item_est_amount_<?php echo $i; ?>"><?php echo $quotation->TAG_ITEMS[$key]->est_amount;  ?></td>
																					<td id="item_delete_<?php echo $i; ?>">
																					
																						<span class="btn btn-icon btn-bg-light  btn-active-color-danger btn-sm"  onclick="item_delete('<?php echo $quotation->TAG_ITEMS[$key]->tag_id;  ?>','<?php echo $i; ?>')" >
																			...
																			<?php }else{?>
																				<tr id="row_<?php echo $i; ?>">
																					...
																					<td id="item_delete_<?php echo $i; ?>"></td>
																				</tr>
												...
												<div class="d-flex justify-content-end mt-4">
													...
													<a href="<?php echo base_url(); ?>Sales_quotation"  class="btn btn-secondary me-3">Cancel</a>
													<button type="submit" class="btn btn-primary" id="">Update Quotation</button>
												</div>
												<!--end::Actions-->
											</div>
											<!--end::Card body-->
		...
		<script>
			...
			$("#sale_party").trigger("autocompleteselect");
		...
		<script>
			...
			let selected_amount = new Array();


			
			<?php foreach($selected_values as $key => $val){ ?>
				selected_values.push('<?php echo $val; ?>');
			<?php } ?>

			<?php foreach($selected_amount as $key => $val){ ?>
				selected_amount.push('<?php echo $val; ?>');
			<?php } ?>
			...
			$('.select_item').on('change', function() {
				...
				if(tag_id){
					...
					if(jQuery.inArray(tag_id,selected_values) != -1) {
						...
						$.ajax({
								...
								success: function(response)
								{
									...
									var html_delete = "<span class='btn btn-icon btn-bg-light  btn-active-color-danger btn-sm'  onclick=item_delete("+"'"+result.tag_id+"'"+','+"'"+index+"'"+")><i class='bi bi-trash-fill trash'></i></span>";
									$('#item_delete_'+index).html(html_delete);
					...
					// var removeItem = 2;   // item do array que deverÃƒÂ¡ ser removido
					
					// selected_values = jQuery.grep(selected_values, function(value) {
		...
		<script>
			function item_delete(item_id,index){

				

				var html_content = "<option value='' selected >Select</option>";

				 <?php foreach($tag_list as $k => $tag_id){ ?>
				 	html_content = html_content +"<option value='"+<?php echo json_encode($tag_id['tag_id']); ?>+"'>"+<?php echo json_encode($tag_id['tag_id']); ?>+"</option>";
				 <?php } ?>




				$("#select_tag_"+index).html(html_content);
