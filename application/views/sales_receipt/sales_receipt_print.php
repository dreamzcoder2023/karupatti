	...
	<body>
		<table id="base">
				...
				<tr>
					...
					<td class="ft_wgt" style="border-bottom-style: hidden;white-space:nowrap;"><span>Bill No &nbsp; <label><?php echo $sales_receipt->sales_bill_id; ?></label></span></td>
				</tr>
				<tr>
					<td class="ft_wgt" style="padding-left:5px;"><span>GST No &nbsp;- <label><?php  echo $company_detail->GST_NO; ?></label></span></td>
					<td class="ft_wgt" style="white-space:nowrap;">
						<span>Date &emsp;&nbsp;&nbsp; <label><?php  $date_format  = $this->db->query("SELECT * FROM general_settings  ")->row();
						 	$format= $date_format->date_format;
							echo date("$format", strtotime($sales_receipt->date)); //echo $sales_receipt->sales_order_date; ?>
							</label>
						</span>
					</td>
				</tr>
		...
		<table id="particular">
			...
			<?php
			$tot_amt=0;
			$i=1; foreach($sales_receipt_tagged as $slist){ ?>
				<tr style="border-bottom-style: hidden;">
					...
					<td><?php
						...
						if(isset($item_name->HSN_CODE)){
						$hsn=$item_name->HSN_CODE;
						}
						else{
							$hsn='';
						}
						echo "-";
						$item_name  = $this->db->query("SELECT * FROM SUBITEM_LIST WHERE  SUB_ID='". $slist['subitem_name']."' ")->row();
						echo $item_name->SUBITEM_NAME;
						?> 
					</td>
					<td align="center"><?php echo $hsn; ?></td>
					<td align="right"><?php echo $slist['metal_weight']; ?></td>
					<td align="right"><?php  echo $slist['current_rate']; ?></td>
					<td align="right"><?php echo number_format($slist['total_amount'],2,'.',','); $tot_amt+=$slist['total_amount']; ?></td>
				</tr>
				<?php $i++; } ?>
<?php for($j=$i-1;$j<=10;$j++){ ?>
			...
			<tfoot>
				<tr>
					...
					<td class="ft_wgt" align="right"> <?php echo number_format($tot_amt,2,'.',','); ?></td>
				</tr>
			</tfoot>
		</table>
		...
		<table id="charges">
			...
			<tr>
				<td class="ft_wgt" style="text-align: center;"><span>Percentage(%)</span></td>
			...
			<tr>
				...
				<td align="right"><?php echo number_format($tot_amt,2,'.',','); ?></td>

				<td align="center"><?php echo "1.5" ?></td>
				<td align="right"><?php $cgst=$tot_amt*1.5/100; echo number_format($cgst,2,'.',','); ?></td>

				<td align="center"><?php echo "1.5" ?></td>
				<td align="right"> <?php $sgst=$tot_amt*1.5/100; echo number_format($sgst,2,'.',','); ?></td>

				<td align="center"><?php echo  "0"; ?></td>
				<td align="right"> <?php echo number_format(0,2,'.',','); ?></td>

				<td align="right"><?php $total=$tot_amt+$cgst+$sgst; echo number_format($total,2,'.',','); ?></td>
			</tr>
		</table>
		<?php
 ...
 ?>
		...
		<table id="decl">
			...
			<tr>
				...
				<td class="ft_wgt" style="border-top-style: hidden;border-bottom-style: hidden;padding-left:5px;"><span>Declaration</span></td>
			</tr>
			<tr>
	...
	<table id="nxt_base">
			...
			<tr>
				...
				<td class="ft_wgt" style="white-space:nowrap;"><span>Date &emsp;&nbsp;&nbsp; <label> <?php $date_format  = $this->db->query("SELECT * FROM general_settings  ")->row();
					 $format= $date_format->date_format;
					 echo date("$format", strtotime($sales_receipt->date)); //echo $sales_order->sales_order_date; ?></label></span>
				</td>
			</tr>
	</table>
				...
				<td style="width:50%">
					<table id="snd_part_item">
							...
							<tr>
								...
								<td>
									<span class="ft_wgt rw"><?php echo number_format($slist['total_amount'],2,'.',',');?></span>
								</td>
							</tr>
					</table>
				</td>
		...
		<table id="indiv_item">
			<tr style="border-top-style: hidden;">
				...
				<?php $image_url =  base_url().'assets/images/sales_order_image/'. $slist['img']; 
					...
					<center>
						<img src="<?php echo base_url(); ?>assets/images/sales_order_image/<?php echo $slist['img']; ?>" width="300" height="300">
					</center>
					<?php }  else {   ?>
